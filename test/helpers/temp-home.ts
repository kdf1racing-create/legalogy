...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    legalogyHome: process.env.LEGALOGY_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("LEGALOGY_HOME", snapshot.legalogyHome);
  restoreKey("LEGALOGY_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked LEGALOGY_HOME.
  delete process.env.LEGALOGY_HOME;
  process.env.LEGALOGY_STATE_DIR = path.join(base, ".legalogy");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "legalogy-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".legalogy", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
