...
import type { LegalogyConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("legalogy-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("legalogy-bin-sherpa-");
      const modelDir = await makeTempDir("legalogy-sherpa-model-");
      ...
      const cfg: LegalogyConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("legalogy-bin-whispercpp-");
      const modelDir = await makeTempDir("legalogy-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("legalogy-bin-gemini-");
      ...
      const cfg: LegalogyConfig = { tools: { media: { image: {} } } };
