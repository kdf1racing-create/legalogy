...
function resolveLegalogyRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.LEGALOGY_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set LEGALOGY_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveLegalogyRoot(), "dist", "extensionAPI.js");
