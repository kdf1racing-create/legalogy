...
import type { LegalogyPluginApi, LegalogyPluginToolContext } from "../../../src/plugins/types.js";
...
function fakeApi(overrides: Partial<LegalogyPluginApi> = {}): LegalogyPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<LegalogyPluginToolContext> = {}): LegalogyPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-lobster-plugin-"));
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: LegalogyPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
