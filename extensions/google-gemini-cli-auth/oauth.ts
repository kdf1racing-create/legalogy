import { isWSL2Sync } from "legalogy/plugin-sdk";
import { createHash, randomBytes } from "node:crypto";
import { existsSync, readFileSync, readdirSync, realpathSync } from "node:fs";
import { createServer } from "node:http";
import { delimiter, dirname, join } from "node:path";

const CLIENT_ID_KEYS = ["LEGALOGY_GEMINI_OAUTH_CLIENT_ID", "GEMINI_CLI_OAUTH_CLIENT_ID"];
const CLIENT_SECRET_KEYS = [
  "LEGALOGY_GEMINI_OAUTH_CLIENT_SECRET",
      ...
      try {
        ...
        res.end(
          "<!doctype html><html><head><meta charset='utf-8'/></head>" +
            ...
            "<p>You can close this window and return to Legalogy.</p></body></html>",
        );
...
async function discoverProject(accessToken: string): Promise<string> {
  ...
  const headers = {
    ...
    "X-Goog-Api-Client": "gl-node/legalogy",
  };
