...
export async function handleIrcInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as LegalogyConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as LegalogyConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as LegalogyConfig);
