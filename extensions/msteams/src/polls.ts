...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["legalogyPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["legalogy", "pollId"]) ??
    readNestedString(value, ["legalogy", "poll", "id"]) ??
    readNestedString(value, ["data", "legalogyPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "legalogy", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          legalogyPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "legalogy poll vote",
          displayText: "Vote recorded",
          value: { legalogyPollId: pollId, pollId },
        },
      },
    ],
  };
