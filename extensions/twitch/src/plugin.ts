/**
 * Twitch channel plugin for Legalogy.
 *
...
import { buildChannelConfigSchema } from "legalogy/plugin-sdk";
import type {
 ...
 * for Legalogy. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: LegalogyConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: LegalogyConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: LegalogyConfig): boolean => {
