import type { LegalogyPluginApi, LegalogyPluginService } from "legalogy/plugin-sdk";
...
function normalizeDenyList(cfg: LegalogyPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: LegalogyPluginApi["config"]): string[] {
...
export default function register(api: LegalogyPluginApi) {
  ...
  const timerService: LegalogyPluginService = {
