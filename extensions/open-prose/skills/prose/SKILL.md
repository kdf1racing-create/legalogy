...
metadata: { "legalogy": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Legalogy `sessions_spawn`
- **File I/O** == Legalogy `read`/`write`
- **Remote fetch** == Legalogy `web_fetch` (or `exec` with curl when POST is required)
