...
Legalogy extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
legalogy channels login --channel zalouser
...
```bash
legalogy message send --channel zalouser --target <threadId> --message "Hello from Legalogy!"
```
...
```bash
legalogy channels login --channel zalouser              # Login via QR
legalogy channels login --channel zalouser --account work
legalogy channels status --probe
legalogy channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
legalogy directory self --channel zalouser
legalogy directory peers list --channel zalouser --query "name"
legalogy directory groups list --channel zalouser --query "work"
legalogy directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
legalogy message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
legalogy message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `legalogy channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
legalogy channels login --channel zalouser --account work
legalogy message send --channel zalouser --account work --target <id> --message "Hello"
