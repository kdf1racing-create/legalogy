...
`legalogy.invoke --each`) without adding new Legalogy code per workflow.
...
This extension depends on Legalogy internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Legalogy extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.legalogy/extensions` as a standalone plugin directory.
