---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Legalogy deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Legalogy cron scheduling for periodic checks, or version status checks on a machine running Legalogy (laptop, workstation, Pi, VPS).
---

# Legalogy Host Hardening

## Overview

Assess and harden the host running Legalogy, then align it to a user-defined risk tolerance without breaking access. Use Legalogy security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Legalogy changes the host firewall, SSH, or OS updates; it does not.
...
5. Legalogy gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Legalogy can access sensitive data.
...
### 2) Run Legalogy security audits (read-only)

As part of the default read-only checks, run `legalogy security audit --deep`. Only offer alternatives if the user requests them:

1. `legalogy security audit` (faster, non-probing)
2. `legalogy security audit --json` (structured output)

Offer to apply Legalogy safe defaults (numbered):

1. `legalogy security audit --fix`

Be explicit that `--fix` only tightens Legalogy defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Legalogy version/update status (read-only)

As part of the default read-only checks, run `legalogy update status`.
...
- Credential hygiene notes (location of Legalogy creds, prefer disk encryption)
...
- Legalogy security audit (re-run)
...
After Legalogy install or first hardening pass, run at least one baseline audit and version check:

- `legalogy security audit`
- `legalogy security audit --deep`
- `legalogy update status`

Ongoing monitoring is recommended. Use the Legalogy cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `legalogy cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `legalogy update status`
...
Before creating, `legalogy cron list` and match on exact `name`. If found, `legalogy cron edit <id> ...`.
If not found, `legalogy cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `legalogy update status` (preferred for source checkouts and channels)
2. `npm view legalogy version` (published npm version)

## Legalogy command accuracy

Use only supported commands and flags:

- `legalogy security audit [--deep] [--fix] [--json]`
- `legalogy status` / `legalogy status --deep`
- `legalogy health --json`
- `legalogy update status`
- `legalogy cron add|list|runs|run`

Do not invent CLI flags or imply Legalogy enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Legalogy compaction:
