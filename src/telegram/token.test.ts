...
function withTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "legalogy-telegram-token-"));
}

async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.LEGALOGY_STATE_DIR;
  const dir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "legalogy-telegram-"));
  process.env.LEGALOGY_STATE_DIR = dir;
  try {
    return await fn(dir);
  } finally {
    if (previous === undefined) {
      ...
      process.env.LEGALOGY_STATE_DIR = previous;
    }
...
describe("resolveTelegramToken", () => {
  ...
  it("uses tokenFile when configured", () => {
    ...
    const cfg = { channels: { telegram: { tokenFile } } } as LegalogyConfig;
    const res = resolveTelegramToken(cfg);
