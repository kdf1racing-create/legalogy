...
function listAgents(cfg: LegalogyConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: LegalogyConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: LegalogyConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<LegalogyConfig, EvaluatedBindingsCache>();
