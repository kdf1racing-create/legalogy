...
import { scheduleGatewaySigusr1Restart, triggerLegalogyRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Legalogy in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerLegalogyRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Legalogy via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
