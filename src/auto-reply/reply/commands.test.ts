...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-commands-"));
...
function buildParams(commandBody: string, cfg: LegalogyConfig, ctxOverrides?: Partial<MsgContext>) {
...
describe("/compact command", () => {
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    ...
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
      session: { store: "/tmp/legalogy-session-store.json" },
    } as LegalogyConfig;
...
describe("handleCommands /allowlist", () => {
  ...
  it("removes Slack DM allowlist entries from canonical allowFrom and deletes legacy dm.allowFrom", async () => {
    ...
    const written = writeConfigFileMock.mock.calls[0]?.[0] as LegalogyConfig;
