...
beforeEach(() => {
  ...
  vi.stubEnv("LEGALOGY_TEST_FAST", "1");
});
...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  function snapshotStateEnv(): StateEnvSnapshot {
    return { LEGALOGY_STATE_DIR: process.env.LEGALOGY_STATE_DIR };
  }

  function restoreStateEnv(snapshot: StateEnvSnapshot) {
    if (snapshot.LEGALOGY_STATE_DIR === undefined) {
      delete process.env.LEGALOGY_STATE_DIR;
    } else {
      process.env.LEGALOGY_STATE_DIR = snapshot.LEGALOGY_STATE_DIR;
    }
  }
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "legalogy-typing-heartbeat-"));
  });
...
describe("runReplyAgent memory flush", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "legalogy-memory-flush-"));
  });
