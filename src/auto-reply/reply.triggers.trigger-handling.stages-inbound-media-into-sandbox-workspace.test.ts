...
describe("stageSandboxMedia", () => {
  it("stages inbound media into the sandbox workspace", async () => {
    await withSandboxMediaTempHome("legalogy-triggers-", async (home) => {
      const inboundDir = join(home, ".legalogy", "media", "inbound");
      ...
      await stageSandboxMedia({
        ...
        workspaceDir: join(home, "legalogy"),
      });
  ...
  it("rejects staging host files from outside the media directory", async () => {
    await withSandboxMediaTempHome("legalogy-triggers-bypass-", async (home) => {
      // Sensitive host file outside .legalogy
