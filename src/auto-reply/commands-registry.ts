...
export function isCommandEnabled(cfg: LegalogyConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: LegalogyConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as LegalogyConfig),
...
export function getCommandDetection(_cfg?: LegalogyConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: LegalogyConfig) {
