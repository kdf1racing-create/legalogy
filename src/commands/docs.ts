...
const SEARCH_TOOL = "https://docs.legalogy.ai/mcp.SearchLegalogy";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.legalogy.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('legalogy docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.legalogy.ai/");
      runtime.log(`Search: ${formatCliCommand('legalogy docs "your query"')}`);
    }
    return;
  }
