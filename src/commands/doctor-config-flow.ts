...
function stripUnknownConfigKeys(config: LegalogyConfig): {
  config: LegalogyConfig;
  removed: string[];
} {
  const parsed = LegalogySchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: LegalogyConfig) {
...
function scanTelegramAllowFromUsernameEntries(cfg: LegalogyConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: LegalogyConfig): Promise<{
  config: LegalogyConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: LegalogyConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: LegalogyConfig): {
  config: LegalogyConfig;
  changes: string[];
} {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".legalogy");
  const targetPath = path.join(targetDir, "legalogy.json");
  try {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: LegalogyConfig = baseCfg;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("legalogy doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("legalogy doctor --fix")}" to apply these changes.`);
    }
  }
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("legalogy doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("legalogy doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "legalogy doctor --fix" to remove these keys.');
    }
  }
