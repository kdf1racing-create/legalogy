...
export function listAgentEntries(cfg: LegalogyConfig): AgentEntry[] {
...
function resolveAgentName(cfg: LegalogyConfig, agentId: string) {
...
function resolveAgentModel(cfg: LegalogyConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: LegalogyConfig): AgentSummary[] {
