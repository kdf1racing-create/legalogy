...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "LEGALOGY_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("LEGALOGY_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv LEGALOGY_GATEWAY_TOKEN");
    expect(message).not.toContain("LEGALOGY_GATEWAY_PASSWORD");
  });
