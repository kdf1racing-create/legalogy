...
export async function updateConfig(
  mutator: (cfg: LegalogyConfig) => LegalogyConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: LegalogyConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: LegalogyConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("legalogy agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
