...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveLegalogyAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("legalogy models auth setup-token")}\` or \`${formatCliCommand("legalogy configure")}\`.`
          : `Run \`${formatCliCommand("legalogy configure")}\` or set an API key env var.`;
