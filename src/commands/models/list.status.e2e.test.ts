...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveLegalogyAgentDir: vi.fn().mockReturnValue("/tmp/legalogy-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/legalogy-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/legalogy-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveLegalogyAgentDir: mocks.resolveLegalogyAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveLegalogyAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/legalogy-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/legalogy-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/legalogy-agent-custom");
