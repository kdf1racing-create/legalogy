...
import type { LegalogyConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: LegalogyConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("legalogy security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("legalogy configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("legalogy config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("legalogy doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("legalogy config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('legalogy config set session.dmScope "per-channel-peer"') +
