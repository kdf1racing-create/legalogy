...
export const resolveLegalogyPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadLegalogyPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/legalogy-root.js", () => ({
  resolveLegalogyPackageRoot,
}));
...
beforeEach(() => {
  ...
  resolveLegalogyPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.LEGALOGY_STATE_DIR;
  originalUpdateInProgress = process.env.LEGALOGY_UPDATE_IN_PROGRESS;
  process.env.LEGALOGY_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "legalogy-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.LEGALOGY_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.LEGALOGY_UPDATE_IN_PROGRESS;
  } else {
    process.env.LEGALOGY_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
