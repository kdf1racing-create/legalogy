...
beforeAll(() => {
  envSnapshot = captureEnv(["LEGALOGY_PROFILE"]);
  process.env.LEGALOGY_PROFILE = "isolated";
});
...
vi.mock("../infra/legalogy-root.js", () => ({
  resolveLegalogyPackageRoot: vi.fn().mockResolvedValue("/tmp/legalogy"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/legalogy/pnpm-lock.yaml",
      markerPath: "/tmp/legalogy/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = (runtime.log as vi.Mock).mock.calls.map((c) => String(c[0]));
    expect(logs.some((l) => l.includes("Legalogy status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("legalogy status --all") ||
          l.includes("legalogy --profile isolated status --all") ||
          l.includes("legalogy status --all") ||
          l.includes("legalogy --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.LEGALOGY_GATEWAY_TOKEN = "abcd1234";
    try {
