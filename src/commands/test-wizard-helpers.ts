...
export function requireLegalogyAgentDir(): string {
  const agentDir = process.env.LEGALOGY_AGENT_DIR;
  if (!agentDir) {
    throw new Error("LEGALOGY_AGENT_DIR not set");
  }
  return agentDir;
}
