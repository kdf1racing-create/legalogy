...
function resolveMode(cfg: LegalogyConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("Legalogy doctor");

  const root = await resolveLegalogyPackageRoot({
  ...
  let cfg: LegalogyConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("legalogy configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("legalogy config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("legalogy setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("legalogy doctor --fix")}" to apply changes.`);
  }
