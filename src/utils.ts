...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".legalogy");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.LEGALOGY_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$LEGALOGY_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via LEGALOGY_STATE_DIR.
