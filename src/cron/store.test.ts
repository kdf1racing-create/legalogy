...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses LEGALOGY_HOME for tilde expansion", () => {
    vi.stubEnv("LEGALOGY_HOME", "/srv/legalogy-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/legalogy-home", "cron", "jobs.json"));
  });
});
