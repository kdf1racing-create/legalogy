...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "legalogy-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    LEGALOGY_GATEWAY_TOKEN: "test-token",
    LEGALOGY_CONFIG_DIR: join(sandbox.rootDir, "config"),
    LEGALOGY_WORKSPACE_DIR: join(sandbox.rootDir, "legalogy"),
  };
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      ...
      env: createEnv(sandbox, {
        LEGALOGY_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
        LEGALOGY_EXTRA_MOUNTS: undefined,
        LEGALOGY_HOME_VOLUME: "legalogy-home",
      }),
      stdio: ["ignore", "ignore", "pipe"],
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("LEGALOGY_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("LEGALOGY_EXTRA_MOUNTS=");
    expect(envFile).toContain("LEGALOGY_HOME_VOLUME=legalogy-home");
    const extraCompose = await readFile(join(sandbox.rootDir, "docker-compose.extra.yml"), "utf8");
    expect(extraCompose).toContain("legalogy-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("legalogy-home:");
    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg LEGALOGY_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
