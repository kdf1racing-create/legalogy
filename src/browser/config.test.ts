...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const legalogy = resolveProfile(resolved, "legalogy");
    expect(legalogy?.driver).toBe("legalogy");
    expect(legalogy?.cdpPort).toBe(18800);
    expect(legalogy?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from LEGALOGY_GATEWAY_PORT when unset", () => {
    const prev = process.env.LEGALOGY_GATEWAY_PORT;
    process.env.LEGALOGY_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const legalogy = resolveProfile(resolved, "legalogy");
      expect(legalogy?.cdpPort).toBe(19012);
      expect(legalogy?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.LEGALOGY_GATEWAY_PORT;
      } else {
        process.env.LEGALOGY_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const legalogy = resolveProfile(resolved, "legalogy");
      expect(legalogy?.cdpPort).toBe(19022);
      expect(legalogy?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.LEGALOGY_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "legalogy");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        legalogy: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("legalogy");
  });
});
