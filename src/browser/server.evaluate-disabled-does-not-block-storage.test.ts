  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          legalogy: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.LEGALOGY_GATEWAY_PORT;
    process.env.LEGALOGY_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.LEGALOGY_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.LEGALOGY_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPort === undefined) {
      ...
      process.env.LEGALOGY_GATEWAY_PORT = prevGatewayPort;
    }
    if (prevGatewayToken === undefined) {
      delete process.env.LEGALOGY_GATEWAY_TOKEN;
    } else {
      process.env.LEGALOGY_GATEWAY_TOKEN = prevGatewayToken;
    }
    if (prevGatewayPassword === undefined) {
      delete process.env.LEGALOGY_GATEWAY_PASSWORD;
    } else {
      process.env.LEGALOGY_GATEWAY_PASSWORD = prevGatewayPassword;
    }
