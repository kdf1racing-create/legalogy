...
vi.mock("./chrome.js", () => ({
  resolveLegalogyUserDataDir: vi.fn(() => "/tmp/legalogy-test/legalogy/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveLegalogyUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "legalogy-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveLegalogyUserDataDir).mockReturnValue(userDataDir);
