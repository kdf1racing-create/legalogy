...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/legalogy" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "legalogy",
        profiles: {
          legalogy: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchLegalogyChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.LEGALOGY_GATEWAY_PORT;
    process.env.LEGALOGY_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.LEGALOGY_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.LEGALOGY_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.LEGALOGY_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.LEGALOGY_GATEWAY_TOKEN;
    } else {
      process.env.LEGALOGY_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.LEGALOGY_GATEWAY_PASSWORD;
    } else {
      process.env.LEGALOGY_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
