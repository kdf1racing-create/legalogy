...
const RELAY_AUTH_HEADER = "x-legalogy-relay-token";
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/Legalogy-Extension-Relay",
          revision: "0",
          userAgent: "Legalogy-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (
      ...
      const payload: Record<string, unknown> = {
        Browser: "Legalogy/extension-relay",
