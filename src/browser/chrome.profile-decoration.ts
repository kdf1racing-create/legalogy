...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".legalogy-profile-decorated");
}
 ...
 */
export function decorateLegalogyProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_LEGALOGY_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_LEGALOGY_BROWSER_COLOR).toUpperCase();
