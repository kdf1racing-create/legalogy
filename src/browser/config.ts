import type { BrowserConfig, BrowserProfileConfig, LegalogyConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_LEGALOGY_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "legalogy" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_LEGALOGY_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "legalogy" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_LEGALOGY_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_LEGALOGY_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Legalogy browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for legalogy/legalogy CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: LegalogyConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_LEGALOGY_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "legalogy";
