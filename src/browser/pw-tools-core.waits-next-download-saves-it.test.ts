...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredLegalogyTmpDir: vi.fn(() => "/tmp/legalogy"),
}));
vi.mock("../infra/tmp-legalogy-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredLegalogyTmpDir.mockReturnValue("/tmp/legalogy");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredLegalogyTmpDir.mockReturnValue("/tmp/legalogy-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "legalogy-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredLegalogyTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "legalogy-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "legalogy-preferred", "downloads")}${path.sep}`),
    );
  });
