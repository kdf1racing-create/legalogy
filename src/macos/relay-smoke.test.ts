...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { LEGALOGY_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { LEGALOGY_SMOKE_QR: "1" })).toBe(null);
  });

  it("supports LEGALOGY_SMOKE=qr only when no args", () => {
    expect(parseRelaySmokeTest([], { LEGALOGY_SMOKE: "qr" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { LEGALOGY_SMOKE: "qr" })).toBe(null);
  });
  ...
  it("prefers explicit --smoke over env vars", () => {
    expect(parseRelaySmokeTest(["--smoke", "qr"], { LEGALOGY_SMOKE: "nope" })).toBe("qr");
  });
});
