...
declare const __LEGALOGY_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __LEGALOGY_VERSION__ === "string" && __LEGALOGY_VERSION__) ||
  process.env.LEGALOGY_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `legalogy --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.LEGALOGY_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.LEGALOGY_GATEWAY_BIND ??
        ...
        if (isRestart) {
          ...
          if (respawn.mode === "spawned" || respawn.mode === "supervised") {
            ...
            if (respawn.mode === "failed") {
              ...
              defaultRuntime.log("gateway: restart mode in-process restart (LEGALOGY_NO_RESPAWN)");
            }
            shuttingDown = false;
            restartResolver?.();
          }
...
void main().catch((err) => {
  console.error(
    "[legalogy] Gateway daemon failed:",
