...
import { resolvePreferredLegalogyTmpDir } from "../infra/tmp-legalogy-dir.js";
import { readLoggingConfig } from "./config.js";
import { type LogLevel, levelToMinLevel, normalizeLogLevel } from "./levels.js";
import { loggingState } from "./state.js";

export const DEFAULT_LOG_DIR = resolvePreferredLegalogyTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "legalogy.log"); // legacy single-file path

const LOG_PREFIX = "legalogy";
...
function resolveSettings(): ResolvedSettings {
  let cfg: LegalogyConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => LegalogyConfig;
      };
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.LEGALOGY_TEST_FILE_LOG !== "1" ? "silent" : "info";
