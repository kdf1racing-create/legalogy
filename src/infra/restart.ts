...
export function triggerLegalogyRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.LEGALOGY_SYSTEMD_UNIT,
        process.env.LEGALOGY_PROFILE,
      );
  ...
  const label =
    process.env.LEGALOGY_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.LEGALOGY_PROFILE);
