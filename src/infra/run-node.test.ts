...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-run-node-"));
  try {
    ...
    async () => {
      await withTempDir(async (tmp) => {
        ...
        const exitCode = await runNodeMain({
          ...
          env: {
            ...process.env,
            LEGALOGY_FORCE_BUILD: "1",
            LEGALOGY_RUNNER_LOG: "0",
          },
          spawn,
        ...
        expect(nodeCalls).toEqual([[process.execPath, "legalogy.mjs", "--version"]]);
      });
    },
  );
});
