...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__legalogy_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "legalogy-root");
...
describe("resolveLegalogyPackageRoot", () => {
  beforeEach(() => {
    state.entries.clear();
  ...
  it("resolves package root from .bin argv1", async () => {
    const { resolveLegalogyPackageRootSync } = await import("./legalogy-root.js");

    const project = fx("bin-scenario");
    const argv1 = path.join(project, "node_modules", ".bin", "legalogy");
    const pkgRoot = path.join(project, "node_modules", "legalogy");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "legalogy" }));

    expect(resolveLegalogyPackageRootSync({ argv1 })).toBe(pkgRoot);
  });
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    const bin = path.join(project, "bin", "legalogy");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "legalogy.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "legalogy" }));

    expect(resolveLegalogyPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveLegalogyPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-legalogy package roots", async () => {
    const { resolveLegalogyPackageRootSync } = await import("./legalogy-root.js");

    const pkgRoot = fx("not-legalogy");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-legalogy" }));

    expect(resolveLegalogyPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });

  it("async resolver matches sync behavior", async () => {
    const { resolveLegalogyPackageRoot } = await import("./legalogy-root.js");

    const pkgRoot = fx("async");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "legalogy" }));

    await expect(resolveLegalogyPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });
});
