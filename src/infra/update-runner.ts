...
const DEFAULT_PACKAGE_NAME = "legalogy";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "legalogy.mjs");
    ...
    const doctorStep = await runStep(
      step("legalogy doctor", doctorArgv, gitRoot, { LEGALOGY_UPDATE_IN_PROGRESS: "1" }),
    );
