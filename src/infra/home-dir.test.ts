...
describe("resolveEffectiveHomeDir", () => {
  it("prefers LEGALOGY_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(
      path.resolve("/srv/legalogy-home"),
    );
  });
  ...
  it("expands LEGALOGY_HOME when set to ~", () => {
    const env = {
      LEGALOGY_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for LEGALOGY_HOME", () => {
    const result = resolveRequiredHomeDir(
      { LEGALOGY_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when LEGALOGY_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ LEGALOGY_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { LEGALOGY_HOME: "/srv/legalogy-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/legalogy-home")}/x`);
  });
