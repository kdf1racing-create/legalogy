...
describe("delivery-queue", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "legalogy-dq-test-"));
  });
...
describe("resolveOutboundSessionRoute", () => {
  const baseConfig = {} as LegalogyConfig;
  ...
  it("treats Telegram usernames as DMs when unresolved", async () => {
    const cfg = { session: { dmScope: "per-channel-peer" } } as LegalogyConfig;
    const route = await resolveOutboundSessionRoute({
      cfg,
      channel: "telegram",
      agentId: "main",
