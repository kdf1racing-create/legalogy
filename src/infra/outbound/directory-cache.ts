...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: LegalogyConfig | null = null;
  ...
  get(key: string, cfg: LegalogyConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: LegalogyConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: LegalogyConfig): void {
