...
describe("update-startup", () => {
  ...
  let resolveLegalogyPackageRoot: (typeof import("./legalogy-root.js"))["resolveLegalogyPackageRoot"];
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    hadStateDir = Object.prototype.hasOwnProperty.call(process.env, "LEGALOGY_STATE_DIR");
    prevStateDir = process.env.LEGALOGY_STATE_DIR;
    process.env.LEGALOGY_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveLegalogyPackageRoot } = await import("./legalogy-root.js"));
  ...
  async function runUpdateCheckAndReadState(channel: "stable" | "beta") {
    vi.mocked(resolveLegalogyPackageRoot).mockResolvedValue("/opt/legalogy");
