...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: LegalogyConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: LegalogyConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: LegalogyConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: LegalogyConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: LegalogyConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: LegalogyConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: LegalogyConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
