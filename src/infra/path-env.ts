...
function candidateBinDirs(opts: EnsureLegalogyPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `legalogy` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "legalogy");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.LEGALOGY_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "legalogy"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `legalogy` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureLegalogyCliOnPath(opts: EnsureLegalogyPathOpts = {}) {
  if (isTruthyEnvValue(process.env.LEGALOGY_PATH_BOOTSTRAPPED)) {
    return;
  }
