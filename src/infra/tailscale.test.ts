...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.LEGALOGY_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.LEGALOGY_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.LEGALOGY_TEST_TAILSCALE_BINARY;
    } else {
      process.env.LEGALOGY_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
