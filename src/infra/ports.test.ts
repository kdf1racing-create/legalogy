...
describe("ports helpers", () => {
  ...
  it("prints an Legalogy-specific hint when port details look like another Legalogy instance", async () => {
    const runtime = {
      error: vi.fn(),
      log: vi.fn(),
      exit: vi.fn() as unknown as (code: number) => never,
    ...
    await handlePortError(
      new PortInUseError(18789, "node dist/index.js legalogy gateway"),
      18789,
      "gateway start",
      runtime,
    ).catch(() => {});
    ...
    const messages = runtime.error.mock.calls.map((call) => stripAnsi(String(call[0] ?? "")));
    expect(messages.join("\n")).toContain("another Legalogy instance is already running");
  });
  ...
  it("classifies ssh and gateway listeners", () => {
    ...
    expect(
      classifyPortListener(
        {
          commandLine: "node /Users/me/Projects/legalogy/dist/entry.js gateway",
        },
        18789,
      ),
    ).toBe("gateway");
