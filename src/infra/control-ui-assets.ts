...
import { resolveLegalogyPackageRoot, resolveLegalogyPackageRootSync } from "./legalogy-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveLegalogyPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "legalogy" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveLegalogyPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // legalogy.mjs or dist/<bundle>.js
