...
let ensureLegalogyCliOnPath: typeof import("./path-env.js").ensureLegalogyCliOnPath;

describe("ensureLegalogyCliOnPath", () => {
  const envKeys = [
    "PATH",
    "LEGALOGY_PATH_BOOTSTRAPPED",
    "LEGALOGY_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureLegalogyCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling legalogy exists", () => {
    const tmp = abs("/tmp/legalogy-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "legalogy");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.LEGALOGY_PATH_BOOTSTRAPPED;

    ensureLegalogyCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.LEGALOGY_PATH_BOOTSTRAPPED = "1";
    ensureLegalogyCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/legalogy-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "legalogy");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/legalogy-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "legalogy");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "legalogy");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/legalogy-path/case-linuxbrew");
