...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.LEGALOGY_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*legalogy-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "legalogy";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "legalogy");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "legalogy-gw");
  ...
  records.push(`_legalogy-gw._tcp IN PTR ${instanceLabel}._legalogy-gw._tcp`);
  records.push(`${instanceLabel}._legalogy-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._legalogy-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; legalogy-content-hash: ${contentHash}\n${contentBody}`;
}
