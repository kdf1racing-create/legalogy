...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("legalogy-ws-", async (workspaceDir) => {
      await withTempDir("legalogy-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createLegalogyCodingTools({ workspaceDir });
    ...
    await withTempDir("legalogy-ws-", async (workspaceDir) => {
      await withTempDir("legalogy-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello legalogy");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("defaults exec cwd to workspaceDir when workdir is omitted", async () => {
    await withTempDir("legalogy-ws-", async (workspaceDir) => {
      const tools = createLegalogyCodingTools({
        workspaceDir,
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("legalogy-ws-", async (workspaceDir) => {
      await withTempDir("legalogy-override-", async (overrideDir) => {
        const tools = createLegalogyCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("legalogy-sandbox-", async (sandboxDir) => {
      await withTempDir("legalogy-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "legalogy-sbx-test",
        ...
        const tools = createLegalogyCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
