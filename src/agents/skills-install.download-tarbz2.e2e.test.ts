...
async function withTempWorkspace(
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-skills-install-"));
  try {
...
function restoreLegalogyStateDir(originalValue: string | undefined): void {
  if (originalValue === undefined) {
    delete process.env.LEGALOGY_STATE_DIR;
    return;
  }
  process.env.LEGALOGY_STATE_DIR = originalValue;
}

const originalStateDir = process.env.LEGALOGY_STATE_DIR;

afterEach(() => {
  restoreLegalogyStateDir(originalStateDir);
});
