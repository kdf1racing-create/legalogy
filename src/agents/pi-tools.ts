...
function resolveExecConfig(params: { cfg?: LegalogyConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: LegalogyConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createLegalogyCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createLegalogyReadTool(freshReadTool);
