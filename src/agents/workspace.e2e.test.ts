...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses LEGALOGY_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      LEGALOGY_HOME: "/srv/legalogy-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/legalogy-home"), ".legalogy", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".legalogy", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("legalogy-workspace-");
