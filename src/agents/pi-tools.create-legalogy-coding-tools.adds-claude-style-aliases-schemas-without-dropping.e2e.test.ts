...
import { createLegalogyTools } from "./legalogy-tools.js";
import { __testing, createLegalogyCodingTools } from "./pi-tools.js";
import { createSandboxedReadTool } from "./pi-tools.read.js";
import { createHostSandboxFsBridge } from "./test-helpers/host-sandbox-fs-bridge.js";
import { createBrowserTool } from "./tools/browser-tool.js";

const defaultTools = createLegalogyCodingTools();
...
describe("createLegalogyCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createLegalogyTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createLegalogyCodingTools({ messageProvider: "discord" });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-depth-policy-"));
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "legalogy-outside.txt");
