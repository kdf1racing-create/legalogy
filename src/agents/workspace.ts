...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.LEGALOGY_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".legalogy", `workspace-${profile}`);
  }
  return path.join(home, ".legalogy", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".legalogy";
