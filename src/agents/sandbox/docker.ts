...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "legalogy.sandbox=1");
  args.push("--label", `legalogy.sessionKey=${params.scopeKey}`);
  args.push("--label", `legalogy.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `legalogy.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "legalogy.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`legalogy sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`legalogy sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("legalogy sandbox recreate --all");
}
