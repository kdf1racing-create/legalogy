...
const originalLegalogyStateDir = process.env.LEGALOGY_STATE_DIR;

afterEach(() => {
  if (originalLegalogyStateDir === undefined) {
    delete process.env.LEGALOGY_STATE_DIR;
  } else {
    process.env.LEGALOGY_STATE_DIR = originalLegalogyStateDir;
  }
});
