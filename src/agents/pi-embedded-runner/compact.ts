...
import { resolveLegalogyDocsPath } from "../docs-path.js";
...
export async function compactEmbeddedPiSessionDirect(
  ...
  const agentDir = params.agentDir ?? resolveLegalogyAgentDir();
  await ensureLegalogyModelsJson(params.config, agentDir);
  ...
  try {
    ...
    const toolsRaw = createLegalogyCodingTools({
      exec: {
    ...
    const promptMode =
      isSubagentSessionKey(params.sessionKey) || isCronSessionKey(params.sessionKey)
        ? "minimal"
        : "full";
    const docsPath = await resolveLegalogyDocsPath({
      workspaceDir: effectiveWorkspace,
