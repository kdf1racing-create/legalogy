...
} from "./legalogy-tools.subagents.sessions-spawn.test-harness.js";
...
type CreateLegalogyTools = (typeof import("./legalogy-tools.js"))["createLegalogyTools"];
type CreateLegalogyToolsOpts = Parameters<CreateLegalogyTools>[0];

async function getSessionsSpawnTool(opts: CreateLegalogyToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createLegalogyTools } = await import("./legalogy-tools.js");
  const tool = createLegalogyTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
...
describe("legalogy-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
  });
