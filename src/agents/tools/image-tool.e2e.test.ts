...
import { createLegalogyCodingTools } from "../pi-tools.js";
...
async function withTempWorkspacePng(
  ...
  const workspaceParent = await fs.mkdtemp(path.join(process.cwd(), ".legalogy-workspace-image-"));
  try {
...
function createMinimaxImageConfig(): LegalogyConfig {
  return {
    agents: {
      defaults: {
...
describe("image tool implicit imageModel config", () => {
  ...
  it("stays disabled without auth when no pairing is possible", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-image-"));
  ...
  it("allows workspace images via createLegalogyCodingTools default workspace root", async () => {
    await withTempWorkspacePng(async ({ imagePath }) => {
      ...
      try {
        ...
        const tools = createLegalogyCodingTools({ config: cfg, agentDir });
  ...
  it("sandboxes image paths like the read tool", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-image-sandbox-"));
  ...
  it("rewrites inbound absolute paths into sandbox media/inbound", async () => {
    ...
    const res = await tool.execute("t1", {
      prompt: "Describe the image.",
      image: "@/Users/steipete/.legalogy/media/inbound/photo.png",
    });
...
describe("image tool MiniMax VLM routing", () => {
  ...
  async function createMinimaxVlmFixture(baseResp: { status_code: number; status_msg: string }) {
    ...
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-minimax-vlm-"));
    vi.stubEnv("MINIMAX_API_KEY", "minimax-test");
    const cfg: LegalogyConfig = {
      agents: { defaults: { model: { primary: "minimax/MiniMax-M2.1" } } },
    };
