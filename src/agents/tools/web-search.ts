...
type WebSearchConfig = NonNullable<LegalogyConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: LegalogyConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("legalogy configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.legalogy.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://legalogy.ai",
      "X-Title": "Legalogy Web Search",
    },
