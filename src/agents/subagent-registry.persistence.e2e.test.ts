...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["LEGALOGY_STATE_DIR"]);
  let tempStateDir: string | null = null;

  const writePersistedRegistry = async (persisted: Record<string, unknown>) => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-subagent-"));
  ...
  it("uses isolated temp state when LEGALOGY_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "legalogy-test-state"));
  });
});
