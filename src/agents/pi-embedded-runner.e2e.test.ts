...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-embedded-agent-"));
...
const makeOpenAiConfig = (modelIds: string[]) =>
  ...
  }) satisfies LegalogyConfig;

const ensureModels = (cfg: LegalogyConfig) => ensureLegalogyModelsJson(cfg, agentDir) as unknown;
