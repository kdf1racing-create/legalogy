...
} from "./legalogy-tools.subagents.test-harness.js";
import "./test-helpers/fast-core-tools.js";

let createLegalogyTools: (typeof import("./legalogy-tools.js"))["createLegalogyTools"];
...
describe("legalogy-tools: subagents steer failure", () => {
  beforeEach(async () => {
    vi.resetModules();
    ({ createLegalogyTools } = await import("./legalogy-tools.js"));
    ...
    const storePath = path.join(
      os.tmpdir(),
      `legalogy-subagents-steer-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
    );
    setSubagentsConfigOverride({
      session: {
        mainKey: "main",
