...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-oauth-"));

    try {
      process.env.LEGALOGY_STATE_DIR = tempDir;
      process.env.LEGALOGY_AGENT_DIR = path.join(tempDir, "agent");
      process.env.PI_CODING_AGENT_DIR = process.env.LEGALOGY_AGENT_DIR;
      ...
      const store = ensureAuthProfileStore(process.env.LEGALOGY_AGENT_DIR, {
  ...
  it("suggests openai-codex when only Codex OAuth is configured", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-auth-"));
