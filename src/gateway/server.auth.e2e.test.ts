...
describe("gateway server auth/connect", () => {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", { timeout: 60_000 }, async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.LEGALOGY_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.LEGALOGY_TEST_HANDSHAKE_TIMEOUT_MS = "50";
      try {
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.LEGALOGY_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.LEGALOGY_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });
    ...
    test("does not grant admin when scopes are omitted", async () => {
      ...
      const identity = loadOrCreateDeviceIdentity(
        path.join(os.tmpdir(), `legalogy-test-device-${randomUUID()}.json`),
      );
  ...
  describe("token auth", () => {
    ...
    beforeAll(async () => {
      prevToken = process.env.LEGALOGY_GATEWAY_TOKEN;
  ...
  test("requires pairing for scope upgrades", async () => {
    ...
    const { getPairedDevice } = await import("../infra/device-pairing.js");
    const { server, ws, port, prevToken } = await startServerWithClient("secret");
    const identityDir = await mkdtemp(join(tmpdir(), "legalogy-device-scope-"));
