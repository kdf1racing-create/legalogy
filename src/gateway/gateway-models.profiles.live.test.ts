...
import type { LegalogyConfig, ModelProviderConfig } from "../config/types.js";
import { resolveLegalogyAgentDir } from "../agents/agent-paths.js";
...
import { ensureLegalogyModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.LEGALOGY_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.LEGALOGY_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.LEGALOGY_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<LegalogyConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.LEGALOGY_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.LEGALOGY_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveLegalogyAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-live-state-"));
  process.env.LEGALOGY_STATE_DIR = tempStateDir;
  ...
  process.env.LEGALOGY_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.legalogy-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveLegalogyAgentDir();
  const sanitizedCfg: LegalogyConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Legalogy live tool probe (local, safe): " +
    ...
    process.env.LEGALOGY_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.LEGALOGY_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureLegalogyModelsJson(cfg);
      ...
      const rawModels = process.env.LEGALOGY_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.legalogy-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
