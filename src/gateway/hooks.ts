...
export function resolveHooksConfig(cfg: LegalogyConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: LegalogyConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-legalogy-token"] === "string"
      ? req.headers["x-legalogy-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
