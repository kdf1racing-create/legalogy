...
const LIVE = isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.LEGALOGY_LIVE_TEST);
const CLI_LIVE = isTruthyEnvValue(process.env.LEGALOGY_LIVE_CLI_BACKEND);
const CLI_IMAGE = isTruthyEnvValue(process.env.LEGALOGY_LIVE_CLI_BACKEND_IMAGE_PROBE);
const CLI_RESUME = isTruthyEnvValue(process.env.LEGALOGY_LIVE_CLI_BACKEND_RESUME_PROBE);
...
function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("LEGALOGY_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}
...
describeLive("gateway live (cli backend)", () => {
  it("runs the agent pipeline against the local CLI backend", async () => {
    const previous = {
      configPath: process.env.LEGALOGY_CONFIG_PATH,
      token: process.env.LEGALOGY_GATEWAY_TOKEN,
      skipChannels: process.env.LEGALOGY_SKIP_CHANNELS,
      skipGmail: process.env.LEGALOGY_SKIP_GMAIL_WATCHER,
      skipCron: process.env.LEGALOGY_SKIP_CRON,
      skipCanvas: process.env.LEGALOGY_SKIP_CANVAS_HOST,
    ...
    process.env.LEGALOGY_GATEWAY_TOKEN = token;

    const rawModel = process.env.LEGALOGY_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
    const parsed = parseModelRef(rawModel, "claude-cli");
    if (!parsed) {
      throw new Error(
        `LEGALOGY_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${rawModel}`,
      );
    }
    ...
    const cliCommand = process.env.LEGALOGY_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
    if (!cliCommand) {
      throw new Error(
        `LEGALOGY_LIVE_CLI_BACKEND_COMMAND is required for provider "${providerId}".`,
      );
    }
    const baseCliArgs =
      parseJsonStringArray(
        "LEGALOGY_LIVE_CLI_BACKEND_ARGS",
        process.env.LEGALOGY_LIVE_CLI_BACKEND_ARGS,
      ) ?? providerDefaults?.args;
    if (!baseCliArgs || baseCliArgs.length === 0) {
      throw new Error(`LEGALOGY_LIVE_CLI_BACKEND_ARGS is required for provider "${providerId}".`);
    }
    const cliClearEnv =
      parseJsonStringArray(
        "LEGALOGY_LIVE_CLI_BACKEND_CLEAR_ENV",
        process.env.LEGALOGY_LIVE_CLI_BACKEND_CLEAR_ENV,
      ) ?? (providerId === "claude-cli" ? DEFAULT_CLEAR_ENV : []);
    const cliImageArg = process.env.LEGALOGY_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || undefined;
    const cliImageMode = parseImageMode(process.env.LEGALOGY_LIVE_CLI_BACKEND_IMAGE_MODE);

    if (cliImageMode && !cliImageArg) {
      throw new Error(
        "LEGALOGY_LIVE_CLI_BACKEND_IMAGE_MODE requires LEGALOGY_LIVE_CLI_BACKEND_IMAGE_ARG.",
      );
    }

    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-live-cli-"));
    const disableMcpConfig = process.env.LEGALOGY_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
    ...
    const tempConfigPath = path.join(tempDir, "legalogy.json");
    await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
    process.env.LEGALOGY_CONFIG_PATH = tempConfigPath;
