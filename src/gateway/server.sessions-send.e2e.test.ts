...
const { createLegalogyTools } = await import("../agents/legalogy-tools.js");
...
beforeAll(async () => {
  envSnapshot = captureEnv(["LEGALOGY_GATEWAY_PORT", "LEGALOGY_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.LEGALOGY_GATEWAY_PORT = String(gatewayPort);
  process.env.LEGALOGY_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createLegalogyTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
...
describe("sessions_send label lookup", () => {
  it("finds session by label and sends message", { timeout: 60_000 }, async () => {
    ...
    if (!configPath) {
      throw new Error("LEGALOGY_CONFIG_PATH missing in gateway test environment");
    }
