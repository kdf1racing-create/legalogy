    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "legalogy", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-legalogy-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-legalogy-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "legalogy", messages: [{ role: "user", content: "hi" }] },
          {
            "x-legalogy-agent-id": "beta",
            "x-legalogy-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
