...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.LEGALOGY_SKIP_CHANNELS;
    prevSkipGmail = process.env.LEGALOGY_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.LEGALOGY_SKIP_PROVIDERS;
    process.env.LEGALOGY_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.LEGALOGY_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.LEGALOGY_SKIP_GMAIL_WATCHER;
    } else {
      process.env.LEGALOGY_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.LEGALOGY_SKIP_PROVIDERS;
    } else {
      process.env.LEGALOGY_SKIP_PROVIDERS = prevSkipProviders;
    }
  });
