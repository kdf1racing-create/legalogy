...
beforeAll(async () => {
  previousToken = process.env.LEGALOGY_GATEWAY_TOKEN;
  process.env.LEGALOGY_GATEWAY_TOKEN = "test-gateway-token-1234567890";
...
afterAll(async () => {
  ...
  if (previousToken === undefined) {
    ...
    process.env.LEGALOGY_GATEWAY_TOKEN = previousToken;
  }
});
...
test.each([
  { clientId: "legalogy-ios", platform: "ios" },
  { clientId: "legalogy-android", platform: "android" },
