...
import { createLegalogyTools } from "../agents/legalogy-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-legalogy-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-legalogy-account-id")?.trim() || undefined;
  ...
  const allTools = createLegalogyTools({
