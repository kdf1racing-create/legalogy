import type { LegalogyConfig } from "../../config/types.legalogy.js";
...
import { loadLegalogyPlugins } from "../../plugins/loader.js";
import {
  ErrorCodes,
  errorShape,
...
function parseValidateConfigFromRawOrRespond(
  ...
  snapshot: Awaited<ReturnType<typeof readConfigFileSnapshot>>,
  respond: RespondFn,
): { config: LegalogyConfig; schema: ConfigSchemaResponse } | null {
...
function loadSchemaWithPlugins(): ConfigSchemaResponse {
  ...
  const pluginRegistry = loadLegalogyPlugins({
    config: cfg,
    cache: true,
    workspaceDir,
    logger: {
  ...
  // our cache. However, both loadConfig() and loadLegalogyPlugins() already cache
