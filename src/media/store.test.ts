...
describe("media store", () => {
  ...
  beforeAll(async () => {
    ...
    home = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-test-home-"));
    ...
    await fs.mkdir(path.join(home, ".legalogy"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.legalogy${path.sep}media`);
