...
function createGroqAudioConfig(): LegalogyConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): LegalogyConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
async function createTempMediaFile(params: { fileName: string; content: Buffer | string }) {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-media-"));
