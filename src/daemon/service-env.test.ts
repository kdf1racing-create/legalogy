...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.LEGALOGY_GATEWAY_PORT).toBe("18789");
    expect(env.LEGALOGY_GATEWAY_TOKEN).toBe("secret");
    expect(env.LEGALOGY_SERVICE_MARKER).toBe("legalogy");
    expect(env.LEGALOGY_SERVICE_KIND).toBe("gateway");
    expect(typeof env.LEGALOGY_SERVICE_VERSION).toBe("string");
    expect(env.LEGALOGY_SYSTEMD_UNIT).toBe("legalogy-gateway.service");
    if (process.platform === "darwin") {
      expect(env.LEGALOGY_LAUNCHD_LABEL).toBe("ai.legalogy.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", LEGALOGY_PROFILE: "work" },
      port: 18789,
    });
    expect(env.LEGALOGY_SYSTEMD_UNIT).toBe("legalogy-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.LEGALOGY_LAUNCHD_LABEL).toBe("ai.legalogy.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".legalogy"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", LEGALOGY_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".legalogy-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", LEGALOGY_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".legalogy"));
  });

  it("uses LEGALOGY_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", LEGALOGY_STATE_DIR: "/var/lib/legalogy" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/legalogy"));
  });

  it("expands ~ in LEGALOGY_STATE_DIR", () => {
    const env = { HOME: "/Users/test", LEGALOGY_STATE_DIR: "~/legalogy-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/legalogy-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { LEGALOGY_STATE_DIR: "C:\\State\\legalogy" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\legalogy");
  });
});
