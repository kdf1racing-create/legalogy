...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.legalogy.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", LEGALOGY_PROFILE: "default" },
    });
...
describe("launchd bootstrap repair", () => {
  it("bootstraps and kickstarts the resolved label", async () => {
    const env: Record<string, string | undefined> = {
      HOME: "/Users/test",
      LEGALOGY_PROFILE: "default",
    };
    const repair = await repairLaunchAgentBootstrap({ env });
    expect(repair.ok).toBe(true);

    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const label = "ai.legalogy.gateway";
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when LEGALOGY_PROFILE is unset", () => {
    const env = { HOME: "/Users/test" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.legalogy.gateway.plist",
    );
  });

  it("uses profile-specific label when LEGALOGY_PROFILE is set to a custom value", () => {
    const env = { HOME: "/Users/test", LEGALOGY_PROFILE: "jbphoenix" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.legalogy.jbphoenix.plist",
    );
  });

  it("prefers LEGALOGY_LAUNCHD_LABEL over LEGALOGY_PROFILE", () => {
    const env = {
      HOME: "/Users/test",
      LEGALOGY_PROFILE: "jbphoenix",
      LEGALOGY_LAUNCHD_LABEL: "com.custom.label",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from LEGALOGY_LAUNCHD_LABEL", () => {
    const env = {
      HOME: "/Users/test",
      LEGALOGY_LAUNCHD_LABEL: "  com.custom.label  ",
    };
  ...
  it("ignores empty LEGALOGY_LAUNCHD_LABEL and falls back to profile", () => {
    const env = {
      HOME: "/Users/test",
      LEGALOGY_PROFILE: "myprofile",
      LEGALOGY_LAUNCHD_LABEL: "   ",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.legalogy.myprofile.plist",
    );
  });
});
