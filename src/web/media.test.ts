...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-media-test-"));
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub LEGALOGY_STATE_DIR.
    // Also keep it outside os.tmpdir() so tmpdir localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["LEGALOGY_STATE_DIR"]);
    process.env.LEGALOGY_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "legalogy-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows default Legalogy state workspace and sandbox roots", async () => {
  ...
  it("rejects default Legalogy state per-agent workspace-* roots without explicit local roots", async () => {
    const { resolveStateDir } = await import("../config/paths.js");
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
