...
describe("isBotMentionedFromTargets", () => {
  const mentionCfg = { mentionRegexes: [/\blegalogy\b/i] };

  it("ignores regex matches when other mentions are present", () => {
    const msg = makeMsg({
      body: "@Legalogy please help",
  ...
  it("ignores JID mentions in self-chat mode", () => {
    const cfg = { mentionRegexes: [/\blegalogy\b/i], allowFrom: ["+999"] };
    const msg = makeMsg({
      body: "@owner ping",
...
describe("resolveMentionTargets with @lid mapping", () => {
  it("resolves mentionedJids via lid reverse mapping in authDir", async () => {
    const authDir = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-lid-mapping-"));
    try {
      await fs.writeFile(
...
describe("getSessionSnapshot", () => {
  it("uses channel reset overrides when configured", async () => {
    ...
    try {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-snapshot-"));
