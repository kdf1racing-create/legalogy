...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    const envSnapshot = captureEnv(["LEGALOGY_BASE_ENV"]);
    process.env.LEGALOGY_BASE_ENV = "base";
    try {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.LEGALOGY_BASE_ENV ?? "") + "|" + (process.env.LEGALOGY_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { LEGALOGY_TEST_ENV: "ok" },
        },
      );
