...
function summarizeGroupPolicy(cfg: LegalogyConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: LegalogyConfig): boolean {
...
function collectModels(cfg: LegalogyConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: LegalogyConfig): Set<string> {
  const baseCfg: LegalogyConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: LegalogyConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: LegalogyConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: LegalogyConfig): boolean {
...
function isBrowserEnabled(cfg: LegalogyConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: LegalogyConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: LegalogyConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep LEGALOGY_STATE_DIR on a local-only volume and re-run "${formatCliCommand("legalogy security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: LegalogyConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer LEGALOGY_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const legalogyGatewayToken =
    typeof env.LEGALOGY_GATEWAY_TOKEN === "string" && env.LEGALOGY_GATEWAY_TOKEN.trim()
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-legalogy-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: LegalogyConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: LegalogyConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
...
export function collectNodeDenyCommandPatternFindings(cfg: LegalogyConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: LegalogyConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: LegalogyConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: LegalogyConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
