...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: LegalogyConfig) => unknown;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-security-audit-"));
  });
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.LEGALOGY_GATEWAY_TOKEN;
    const prevPassword = process.env.LEGALOGY_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.LEGALOGY_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.LEGALOGY_GATEWAY_PASSWORD;
      } else {
        process.env.LEGALOGY_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    ...
    const configPath = path.join(stateDir, "legalogy.json");
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: LegalogyConfig = { session: { dmScope: "main" } };
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    const prevStateDir = process.env.LEGALOGY_STATE_DIR;
    const tmp = await makeTmpDir("discord");
    process.env.LEGALOGY_STATE_DIR = tmp;
    ...
    try {
      ...
      if (prevStateDir == null) {
        ...
        process.env.LEGALOGY_STATE_DIR = prevStateDir;
      }
    }
  });
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: LegalogyConfig = { gateway: { mode: "local" } };
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    const prevToken = process.env.LEGALOGY_GATEWAY_TOKEN;
    process.env.LEGALOGY_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: LegalogyConfig = {
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      includeFilesystem: false,
      includeChannelSecurity: false,
      stateDir: "/Users/test/Dropbox/.legalogy",
      configPath: "/Users/test/Dropbox/.legalogy/legalogy.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    try {
      const cfg: LegalogyConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "legalogy.json"),
      });
  ...
  it("flags plugins with dangerous code patterns (deep audit)", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        legalogy: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  it("flags plugin extension entry path traversal in deep audit", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "escape-plugin",
        legalogy: { extensions: ["../outside.js"] },
      }),
    );
  ...
  describe("maybeProbeGateway auth selection", () => {
    const originalEnvToken = process.env.LEGALOGY_GATEWAY_TOKEN;
    const originalEnvPassword = process.env.LEGALOGY_GATEWAY_PASSWORD;
    ...
    afterEach(() => {
      if (originalEnvToken == null) {
        ...
        process.env.LEGALOGY_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword == null) {
        delete process.env.LEGALOGY_GATEWAY_PASSWORD;
      } else {
        process.env.LEGALOGY_GATEWAY_PASSWORD = originalEnvPassword;
      }
    });
    ...
    it("prefers env token over local config token", async () => {
      process.env.LEGALOGY_GATEWAY_TOKEN = "env-token";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: LegalogyConfig = {
        gateway: {
          mode: "local",
    ...
    it("prefers env password over remote password", async () => {
      process.env.LEGALOGY_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: LegalogyConfig = {
        gateway: {
          mode: "remote",
