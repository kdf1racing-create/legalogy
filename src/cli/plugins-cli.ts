...
function applySlotSelectionForPlugin(
  ...
  pluginId: string,
): { config: LegalogyConfig; warnings: string[] } {
...
function enablePluginInConfig(config: LegalogyConfig, pluginId: string): LegalogyConfig {
  return {
    ...config,
    plugins: {
      ...config.plugins,
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage Legalogy plugins/extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.legalogy.ai/cli/plugins")}\n`,
    );
    ...
    .action(async (raw: string, opts: { link?: boolean }) => {
      ...
      if (fs.existsSync(resolved)) {
        if (opts.link) {
          ...
          let next: LegalogyConfig = enablePluginInConfig(
            {
              ...cfg,
              plugins: {
                ...cfg.plugins,
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.legalogy.ai/plugin");
      lines.push("");
