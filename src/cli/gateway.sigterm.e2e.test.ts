...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "legalogy-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      LEGALOGY_NO_RESPAWN: "1",
      LEGALOGY_STATE_DIR: stateDir,
      LEGALOGY_SKIP_CHANNELS: "1",
      LEGALOGY_SKIP_GMAIL_WATCHER: "1",
      LEGALOGY_SKIP_CRON: "1",
      LEGALOGY_SKIP_BROWSER_CONTROL_SERVER: "1",
      LEGALOGY_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "legalogy-entry-bootstrap.mjs");
