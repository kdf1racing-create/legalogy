...
import { resolveLegalogyPackageRoot } from "../../infra/legalogy-root.js";
...
const LEGALOGY_REPO_URL = "https://github.com/legalogy/legalogy.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "legalogy";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("legalogy@")) {
    return trimmed.slice("legalogy@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.LEGALOGY_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveLegalogyPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", LEGALOGY_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `LEGALOGY_GIT_DIR points at a non-git directory: ${params.dir}. Set LEGALOGY_GIT_DIR to an empty folder or an legalogy checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`LEGALOGY_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "legalogy.mjs");
