...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    LEGALOGY_STATE_DIR: process.env.LEGALOGY_STATE_DIR,
    LEGALOGY_CONFIG_PATH: process.env.LEGALOGY_CONFIG_PATH,
    LEGALOGY_GATEWAY_PORT: process.env.LEGALOGY_GATEWAY_PORT,
    LEGALOGY_PROFILE: process.env.LEGALOGY_PROFILE,
  };

  beforeEach(() => {
    process.env.LEGALOGY_STATE_DIR = "/tmp/legalogy-cli-state";
    process.env.LEGALOGY_CONFIG_PATH = "/tmp/legalogy-cli-state/legalogy.json";
    delete process.env.LEGALOGY_GATEWAY_PORT;
    delete process.env.LEGALOGY_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.LEGALOGY_STATE_DIR !== undefined) {
      process.env.LEGALOGY_STATE_DIR = originalEnv.LEGALOGY_STATE_DIR;
    } else {
      delete process.env.LEGALOGY_STATE_DIR;
    }

    if (originalEnv.LEGALOGY_CONFIG_PATH !== undefined) {
      process.env.LEGALOGY_CONFIG_PATH = originalEnv.LEGALOGY_CONFIG_PATH;
    } else {
      delete process.env.LEGALOGY_CONFIG_PATH;
    }

    if (originalEnv.LEGALOGY_GATEWAY_PORT !== undefined) {
      process.env.LEGALOGY_GATEWAY_PORT = originalEnv.LEGALOGY_GATEWAY_PORT;
    } else {
      delete process.env.LEGALOGY_GATEWAY_PORT;
    }

    if (originalEnv.LEGALOGY_PROFILE !== undefined) {
      process.env.LEGALOGY_PROFILE = originalEnv.LEGALOGY_PROFILE;
    } else {
      delete process.env.LEGALOGY_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        LEGALOGY_PROFILE: "dev",
        LEGALOGY_STATE_DIR: "/tmp/legalogy-daemon-state",
        LEGALOGY_CONFIG_PATH: "/tmp/legalogy-daemon-state/legalogy.json",
        LEGALOGY_GATEWAY_PORT: "19001",
      },
