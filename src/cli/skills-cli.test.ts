...
describe("skills-cli", () => {
  ...
  describe("integration: loads real skills from bundled directory", () => {
    ...
    beforeAll(async () => {
      envSnapshot = captureEnv(["LEGALOGY_BUNDLED_SKILLS_DIR"]);
      tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "legalogy-skills-test-"));
      tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "legalogy-bundled-skills-test-"));
      process.env.LEGALOGY_BUNDLED_SKILLS_DIR = tempBundledDir;
