...
${formatHelpExamples([
  ['legalogy agent --to +15555550123 --message "status update"', "Start a new session."],
  ['legalogy agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'legalogy agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'legalogy agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['legalogy agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'legalogy agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.legalogy.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.legalogy.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['legalogy agents set-identity --agent main --name "Legalogy" --emoji "🦞"', "Set name + emoji."],
  ["legalogy agents set-identity --agent main --avatar avatars/legalogy.png", "Set avatar path."],
  [
    "legalogy agents set-identity --workspace ~/.legalogy/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "legalogy agents set-identity --identity-file ~/.legalogy/workspace/IDENTITY.md --agent main",
