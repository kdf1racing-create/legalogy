...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("legalogy node install"), formatCliCommand("legalogy node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("legalogy node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    LEGALOGY_LOG_PREFIX: baseEnv.LEGALOGY_LOG_PREFIX ?? "node",
