...
import type { ConfigFileSnapshot, LegalogyConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `legalogy config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<[], Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<[LegalogyConfig], Promise<void>>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: LegalogyConfig) => mockWriteConfigFile(cfg),
}));
...
function setSnapshot(resolved: LegalogyConfig, config: LegalogyConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: LegalogyConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
