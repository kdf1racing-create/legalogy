...
vi.mock("../infra/legalogy-root.js", () => ({
  resolveLegalogyPackageRoot: vi.fn(),
}));
...
const { resolveLegalogyPackageRoot } = await import("../infra/legalogy-root.js");
...
describe("update-cli", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-update-tests-"));
  });
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = await createCaseDir("legalogy-update");
    setTty(false);
    readPackageVersion.mockResolvedValue("2.0.0");

    vi.mocked(resolveLegalogyPackageRoot).mockResolvedValue(tempDir);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveLegalogyPackageRoot).mockReset();
    ...
    vi.mocked(resolveLegalogyPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("legalogy");
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("Legalogy update status");
  });
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await createCaseDir("legalogy-update-wizard");
    const envSnapshot = captureEnv(["LEGALOGY_GIT_DIR"]);
    try {
      setTty(true);
      process.env.LEGALOGY_GIT_DIR = tempDir;
