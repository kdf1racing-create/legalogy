...
describe("agent components", () => {
  const createCfg = (): LegalogyConfig => ({}) as LegalogyConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): LegalogyConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockReset().mockReturnValue("/tmp/legalogy-sessions-test.json");
  });
