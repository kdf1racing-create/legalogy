import type { LegalogyConfig } from "./config.js";
...
function isTelegramConfigured(cfg: LegalogyConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: LegalogyConfig, env: NodeJS.ProcessEnv): boolean {
...
function isIrcConfigured(cfg: LegalogyConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: LegalogyConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: LegalogyConfig): boolean {
...
function isIMessageConfigured(cfg: LegalogyConfig): boolean {
...
function isWhatsAppConfigured(cfg: LegalogyConfig): boolean {
...
function isGenericChannelConfigured(cfg: LegalogyConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: LegalogyConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: LegalogyConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: LegalogyConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: LegalogyConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: LegalogyConfig, pluginId: string): LegalogyConfig {
...
function registerPluginEntry(cfg: LegalogyConfig, pluginId: string): LegalogyConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
