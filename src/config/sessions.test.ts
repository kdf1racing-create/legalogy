...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-sessions-suite-"));
  });
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-legalogy",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-legalogy#general");
  });
  ...
  it("derives session transcripts dir from LEGALOGY_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { LEGALOGY_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.LEGALOGY_STATE_DIR = "/custom/state";
    try {
