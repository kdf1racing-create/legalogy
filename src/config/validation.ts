...
import type { LegalogyConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: LegalogyConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: LegalogyConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = LegalogySchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as LegalogyConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as LegalogyConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as LegalogyConfig,
  };
}
