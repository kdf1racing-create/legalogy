...
const ETC_LEGALOGY_DIR = path.join(ROOT_DIR, "etc", "legalogy");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "legalogy.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcLegalogyPath(...parts: string[]) {
  return path.join(ETC_LEGALOGY_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcLegalogyPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "legalogy.json"))).toEqual({
      shared: true,
    });
  });
});
