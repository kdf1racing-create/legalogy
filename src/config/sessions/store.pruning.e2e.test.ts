...
// Keep integration tests deterministic: never read a real legalogy.json.
vi.mock("../config.js", () => ({
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-pruning-integ-"));
  });
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.LEGALOGY_SESSION_CACHE_TTL_MS;
    process.env.LEGALOGY_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.LEGALOGY_SESSION_CACHE_TTL_MS;
    } else {
      process.env.LEGALOGY_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
