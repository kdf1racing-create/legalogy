...
import type { LegalogyConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { compareLegalogyVersions } from "./version.js";
...
function coerceConfig(value: unknown): LegalogyConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as LegalogyConfig;
}
...
function stampConfigVersion(cfg: LegalogyConfig): LegalogyConfig {
...
function warnIfConfigFromFuture(cfg: LegalogyConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareLegalogyVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Legalogy (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as LegalogyConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): LegalogyConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as LegalogyConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: LegalogyConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as LegalogyConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.LEGALOGY_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.LEGALOGY_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.LEGALOGY_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.LEGALOGY_WATCH_SESSION === "string" &&
        deps.env.LEGALOGY_WATCH_SESSION.trim().length > 0
          ? deps.env.LEGALOGY_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.LEGALOGY_WATCH_COMMAND === "string" &&
        deps.env.LEGALOGY_WATCH_COMMAND.trim().length > 0
          ? deps.env.LEGALOGY_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `LEGALOGY_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.LEGALOGY_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.LEGALOGY_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): LegalogyConfig {
