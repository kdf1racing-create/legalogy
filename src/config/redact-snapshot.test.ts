...
import type { ConfigFileSnapshot } from "./types.legalogy.js";
import {
  REDACTED_SENTINEL,
...
function makeSnapshot(config: Record<string, unknown>, raw?: string): ConfigFileSnapshot {
  return {
    path: "/home/user/.legalogy/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/legalogy/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/legalogy/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/legalogy/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/legalogy/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    ...
    const hints = mapSensitivePaths(LegalogySchema, "", {});
