...
import { loadLegalogyPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `legalogy-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.LEGALOGY_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "legalogy.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.LEGALOGY_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadLegalogyPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    ...
    process.env.LEGALOGY_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadLegalogyPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadLegalogyPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadLegalogyPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
