...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `legalogy-plugin-install-${randomUUID()}`);
    ...
    JSON.stringify(
      {
        ...
        legalogy: { extensions: params.extensions },
      },
      null,
      2,
    ),
...
async function expectArchiveInstallReservedSegmentRejection(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      version: "0.0.1",
      legalogy: { extensions: ["./dist/index.js"] },
    }),
    "utf-8",
  );
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.legalogy/extensions and uses unscoped id", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
  ...
  it("rejects packages without legalogy.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@legalogy/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toContain("legalogy.extensions");
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    const result = await installPluginFromNpmSpec({
      spec: "@legalogy/voice-call@0.0.1",
      extensionsDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(argv).toEqual(["npm", "pack", "@legalogy/voice-call@0.0.1", "--ignore-scripts"]);
