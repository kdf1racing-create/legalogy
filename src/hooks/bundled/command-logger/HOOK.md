...
homepage: https://docs.legalogy.ai/automation/hooks#command-logger
metadata:
  {
    "legalogy":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.legalogy/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.legalogy/logs/commands.log ~/.legalogy/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/legalogy`:
   ```
   /home/username/.legalogy/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
