...
This directory contains hooks that ship with Legalogy. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.legalogy/legalogy.json`:
...
2. Restart gateway: `pkill -9 -f 'legalogy.*gateway' && pnpm legalogy gateway`
3. Enable the hook: `legalogy hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.legalogy.ai/automation/hooks
