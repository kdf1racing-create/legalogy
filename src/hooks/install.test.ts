...
const fixtureRoot = path.join(os.tmpdir(), `legalogy-hook-install-${randomUUID()}`);
let tempDirIndex = 0;
...
describe("installHooksFromPath", () => {
  it("uses --ignore-scripts for dependency install", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@legalogy/test-hooks",
        version: "0.0.1",
        legalogy: { hooks: ["./hooks/one-hook"] },
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'metadata: {"legalogy":{"events":["command:new"]}}',
        "---",
        "",
        "# One Hook",
      ].join("\n"),
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    const result = await installHooksFromNpmSpec({
      spec: "@legalogy/test-hooks@0.0.1",
      hooksDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(argv).toEqual(["npm", "pack", "@legalogy/test-hooks@0.0.1", "--ignore-scripts"]);
