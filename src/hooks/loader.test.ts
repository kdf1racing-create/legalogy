...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    originalBundledDir = process.env.LEGALOGY_BUNDLED_HOOKS_DIR;
    process.env.LEGALOGY_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(async () => {
    ...
    if (originalBundledDir === undefined) {
      delete process.env.LEGALOGY_BUNDLED_HOOKS_DIR;
    } else {
      process.env.LEGALOGY_BUNDLED_HOOKS_DIR = originalBundledDir;
    }
  });
