...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "legalogy-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "legalogy-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "legalogy-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "legalogy-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.legalogy when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "legalogy-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers LEGALOGY_HOME over HOME", () => {
    vi.stubEnv("LEGALOGY_HOME", "/srv/legalogy-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/legalogy-home"));
...
describe("shortenHomePath", () => {
  it("uses $LEGALOGY_HOME prefix when LEGALOGY_HOME is set", () => {
    vi.stubEnv("LEGALOGY_HOME", "/srv/legalogy-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/legalogy-home")}/.legalogy/legalogy.json`)).toBe(
      "$LEGALOGY_HOME/.legalogy/legalogy.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $LEGALOGY_HOME replacement when LEGALOGY_HOME is set", () => {
    vi.stubEnv("LEGALOGY_HOME", "/srv/legalogy-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/legalogy-home")}/.legalogy/legalogy.json`),
    ).toBe("config: $LEGALOGY_HOME/.legalogy/legalogy.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/legalogy")).toBe(path.resolve(os.homedir(), "legalogy"));
  });
  ...
  it("prefers LEGALOGY_HOME for tilde expansion", () => {
    vi.stubEnv("LEGALOGY_HOME", "/srv/legalogy-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/legalogy")).toBe(path.resolve("/srv/legalogy-home", "legalogy"));
