...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.legalogyCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.legalogyCanvasA2UIAction &&
        typeof window.legalogyCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.legalogySendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("legalogy:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Legalogy node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.legalogySendUserAction === "function"
        ? window.legalogySendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.LEGALOGY_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.LEGALOGY_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Legalogy Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
