...
describe("canvas host state dir defaults", () => {
  ...
  it("uses LEGALOGY_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "legalogy-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("Legalogy Canvas");
    } finally {
      await handler.close();
