...
describe("shared/frontmatter", () => {
  ...
  test("resolveLegalogyManifestBlock parses JSON5 metadata and picks legalogy block", () => {
    const frontmatter = {
      metadata: "{ legalogy: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveLegalogyManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveLegalogyManifestBlock returns undefined for invalid input", () => {
    expect(resolveLegalogyManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveLegalogyManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveLegalogyManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
