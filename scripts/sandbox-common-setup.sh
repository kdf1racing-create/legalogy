...
BASE_IMAGE="${BASE_IMAGE:-legalogy-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-legalogy-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=legalogy.sandbox=1)
NOTE
