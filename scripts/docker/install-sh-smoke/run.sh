...
INSTALL_URL="${LEGALOGY_INSTALL_URL:-https://legalogy.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${LEGALOGY_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${LEGALOGY_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="legalogy"
PACKAGE_NAME="${LEGALOGY_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (LEGALOGY_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${LEGALOGY_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${LEGALOGY_INSTALL_LATEST_OUT:-}"
fi
