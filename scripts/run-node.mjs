...
const shouldBuild = (deps) => {
  if (deps.env.LEGALOGY_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.LEGALOGY_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[legalogy] ${message}\n`);
};

const runLegalogy = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["legalogy.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
