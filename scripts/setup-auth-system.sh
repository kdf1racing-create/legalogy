#!/bin/bash
# Setup Legalogy Auth Management System
...
echo "=== Legalogy Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'legalogy-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Legalogy message: Send warning via Legalogy itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/legalogy-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/legalogy-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/legalogy-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now legalogy-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status legalogy-auth-monitor.timer"
echo ""
echo "Quick commands:"
