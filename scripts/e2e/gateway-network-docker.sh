...
IMAGE_NAME="legalogy-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="legalogy-net-e2e-$$"
GW_NAME="legalogy-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "LEGALOGY_GATEWAY_TOKEN=$TOKEN" \
	  -e "LEGALOGY_SKIP_CHANNELS=1" \
	  -e "LEGALOGY_SKIP_GMAIL_WATCHER=1" \
	  -e "LEGALOGY_SKIP_CRON=1" \
	  -e "LEGALOGY_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
