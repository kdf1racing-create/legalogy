...
#
# Suggested cron: */30 * * * * /home/admin/legalogy/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Legalogy notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., legalogy-alerts)
...
STATE_FILE="$HOME/.legalogy/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Legalogy if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use legalogy
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Legalogy to $NOTIFY_PHONE..."
            legalogy send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Legalogy is down. Run: ssh l36 '~/legalogy/scripts/mobile-reauth.sh'" "urgent"
    exit 1
