...
# Build and bundle Legalogy into a minimal .app we can open.
# Outputs to dist/Legalogy.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Legalogy.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Legalogy"
BUNDLE_ID="${BUNDLE_ID:-ai.legalogy.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/legalogy/legalogy/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Legalogy/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :LegalogyBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :LegalogyGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Legalogy"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Legalogy"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Legalogy"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Legalogy" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Legalogy/Resources/Legalogy.icns" "$APP_ROOT/Contents/Resources/Legalogy.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Legalogy/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying LegalogyKit resources"
LEGALOGYKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/LegalogyKit_LegalogyKit.bundle"
if [ -d "$LEGALOGYKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/LegalogyKit_LegalogyKit.bundle"
  cp -R "$LEGALOGYKIT_BUNDLE" "$APP_ROOT/Contents/Resources/LegalogyKit_LegalogyKit.bundle"
else
  echo "WARN: LegalogyKit resource bundle not found at $LEGALOGYKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Legalogy"
killall -q Legalogy 2>/dev/null || true
