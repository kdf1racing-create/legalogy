...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/legalogy/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Legalogy Re-Auth" -c "After getting key, run: ssh $SERVER '~/legalogy/scripts/mobile-reauth.sh'" --id legalogy-auth
        ;;
    *)
