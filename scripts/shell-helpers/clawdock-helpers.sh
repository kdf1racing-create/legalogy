#!/usr/bin/env bash
# ClawDock - Docker helpers for Legalogy
# Inspired by Simon Willison's "Running Legalogy in Docker"
# https://til.simonwillison.net/llms/legalogy-docker
#
# Installation:
...
# Common paths to check for Legalogy
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/legalogy"
  "${HOME}/workspace/legalogy"
  "${HOME}/projects/legalogy"
  "${HOME}/dev/legalogy"
  "${HOME}/code/legalogy"
  "${HOME}/src/legalogy"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Legalogy at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/legalogy"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Legalogy not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/legalogy/legalogy.git ~/legalogy"
    echo "  cd ~/legalogy && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^LEGALOGY_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d legalogy-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart legalogy-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f legalogy-gateway
}
...
clawdock-workspace() {
  cd ~/.legalogy/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec legalogy-gateway \
    bash -c 'echo "alias legalogy=\"./legalogy.mjs\"" > /tmp/.bashrc_legalogy && bash --rcfile /tmp/.bashrc_legalogy'
}
...
clawdock-cli() {
  _clawdock_compose run --rm legalogy-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build legalogy-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "LEGALOGY_GATEWAY_TOKEN=$token" legalogy-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" legalogy-gateway \
    bash -c './legalogy.mjs config set gateway.remote.token "$TOKEN" && ./legalogy.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec legalogy-gateway \
    bash -c "./legalogy.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart legalogy-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm legalogy-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec legalogy-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'legalogy config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Legalogy${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (legalogy alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to legalogy project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.legalogy)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'legalogy channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'legalogy status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.legalogy.ai${_CLR_RESET}"
  echo ""
}
