...
ZIP=${1:?"Usage: $0 Legalogy-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/legalogy/legalogy/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Legalogy-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/legalogy/legalogy/releases/download/v${VERSION}/"}
