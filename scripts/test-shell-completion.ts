 ...
 * `legalogy update`. Use it to verify the completion installation flow
...
\`legalogy update\`, \`legalogy doctor\`, and \`legalogy onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Legalogy Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`legalogy completion --install\` later to enable.`));
    return;
  }
