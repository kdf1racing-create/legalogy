...
- Jobs persist under `~/.legalogy/cron/` so restarts don’t lose schedules.
...
legalogy cron list
legalogy cron run <job-id>
legalogy cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.legalogy/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `legalogy cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, Legalogy defaults to `announce`.
...
- Job store: `~/.legalogy/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.legalogy/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
legalogy cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
legalogy cron edit <jobId> --agent ops
legalogy cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
legalogy cron run <jobId>
legalogy cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
legalogy cron edit <jobId> \
...
```bash
legalogy system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`legalogy system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `LEGALOGY_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Legalogy applies exponential retry backoff for recurring jobs after consecutive errors:
