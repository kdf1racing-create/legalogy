...
read_when:
  - You want to place an outbound voice call from Legalogy
...
Voice calls for Legalogy via a plugin. Supports outbound notifications and
...
- Use `legalogy voicecall ...` or the `voice_call` tool
...
```bash
legalogy plugins install ./extensions/voice-call
