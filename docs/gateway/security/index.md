...
## Quick check: `legalogy security audit`
...
```bash
legalogy security audit
legalogy security audit --deep
legalogy security audit --fix
```
...
- Tighten local perms (`~/.legalogy` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Here’s how to not get pwned.

Legalogy is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, Legalogy also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.legalogy/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.legalogy/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.legalogy/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.legalogy/credentials/oauth.json`
...
`legalogy security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${LEGALOGY_GATEWAY_PASSWORD}
```
...
Legalogy stores session transcripts on disk under `~/.legalogy/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.legalogy` (see the audit section below). If you need
...
Legalogy can refresh the skills list mid-session:
...
- If you install plugins from npm (`legalogy plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.legalogy/extensions/<pluginId>/` (or `$LEGALOGY_STATE_DIR/extensions/<pluginId>/`).
  - Legalogy uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
legalogy pairing list <channel>
legalogy pairing approve <channel> <code>
```
...
By default, Legalogy routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Legalogy has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.legalogy/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.legalogy or your logs.”
...
4. **Re-run audit**
   - `legalogy security audit --deep` and confirm the report is clean.
...
- `~/.legalogy/legalogy.json`: `600` (user read/write only)
- `~/.legalogy`: `700` (user only)

`legalogy doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `LEGALOGY_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__legalogy__/canvas/` and `/__legalogy__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_legalogy-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `LEGALOGY_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `legalogy doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `LEGALOGY_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `LEGALOGY_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Legalogy
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. Legalogy verifies the identity by resolving the
...
- Legalogy will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.legalogy/` (or `$LEGALOGY_STATE_DIR/`) may contain secrets or private data:

- `legalogy.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `legalogy status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@legalogy", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.legalogy/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `legalogy` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only Legalogy clients can connect.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default Legalogy limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `legalogy gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `LEGALOGY_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/legalogy/legalogy-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.legalogy/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + Legalogy version
...
Found a vulnerability in Legalogy? Please report responsibly:

1. Email: [security@legalogy.ai](mailto:security@legalogy.ai)
