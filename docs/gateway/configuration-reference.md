...
description: "Complete field-by-field reference for ~/.legalogy/legalogy.json"
---

# Configuration Reference

Every field available in `~/.legalogy/legalogy.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Legalogy uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `legalogy doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["legalogy-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `legalogy plugins install @legalogy/mattermost`.
...
Legalogy spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@legalogy"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `legalogy.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, Legalogy auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Legalogy assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.legalogy/sandboxes`
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.legalogy/workspace",
        agentDir: "~/.legalogy/agents/main/agent",
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:legalogy`   | All built-in tools (excludes provider plugins)                                           |
...
Legalogy uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.legalogy/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `LEGALOGY_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `legalogy onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `legalogy onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `legalogy onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `legalogy onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `legalogy onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `legalogy onboard --auth-choice minimax-api`.
...
- Loaded from `~/.legalogy/extensions`, `<workspace>/.legalogy/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or LEGALOGY_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `LEGALOGY_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.legalogy-dev` + port `19001`), `--profile <name>` (uses `~/.legalogy-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.legalogy/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-legalogy-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `LEGALOGY_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or LEGALOGY_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__legalogy__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__legalogy__/a2ui/`
...
- Also serves A2UI at `/__legalogy__/a2ui/`.
...
- Hostname defaults to `legalogy`. Override with `LEGALOGY_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.legalogy/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `legalogy dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.legalogy/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${LEGALOGY_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.legalogy/credentials/oauth.json`.
...
- Default log file: `/tmp/legalogy/legalogy-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `legalogy doctor --fix` can strip unknown keys).
