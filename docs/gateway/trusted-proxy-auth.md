...
read_when:
  - Running Legalogy behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Legalogy
...
- You run Legalogy behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Legalogy checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Legalogy extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://legalogy.example.com
    to: http://legalogy-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
legalogy.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy legalogy:18789 {
...
location / {
    ...
    proxy_pass http://legalogy:18789;
...
`legalogy security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Legalogy config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `legalogy security audit` and review findings
