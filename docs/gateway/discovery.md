...
Legalogy has two distinct problems that look similar on the surface:

1. **Operator remote control**: the macOS menu bar app controlling a gateway running elsewhere.
2. **Node pairing**: iOS/Android (and future nodes) finding a gateway and pairing securely.

The design goal is to keep all network discovery/advertising in the **Node Gateway** (`legalogy gateway`) and keep clients (mac app, iOS) as consumers.
...
- Service types:
  - `_legalogy-gw._tcp` (gateway transport beacon)
- TXT keys (non-secret):
  - `role=gateway`
  ...
  - `cliPath=<path>` (optional; absolute path to a runnable `legalogy` entrypoint or binary)
...
- `LEGALOGY_DISABLE_BONJOUR=1` disables advertising.
- `gateway.bind` in `~/.legalogy/legalogy.json` controls the Gateway bind mode.
- `LEGALOGY_SSH_PORT` overrides the SSH port advertised in TXT (defaults to 22).
- `LEGALOGY_TAILNET_DNS` publishes a `tailnetDns` hint (MagicDNS).
- `LEGALOGY_CLI_PATH` overrides the advertised CLI path.
