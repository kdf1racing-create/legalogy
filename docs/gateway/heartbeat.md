...
- During heartbeat runs, Legalogy treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `legalogy sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Legalogy skips the heartbeat run entirely (no model call).
...
```bash
legalogy system event --text "Check for urgent follow-ups" --mode now
```
