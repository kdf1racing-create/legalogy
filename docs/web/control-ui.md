...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/legalogy`)
...
If the page fails to load, start the Gateway first: `legalogy gateway`.
...
you revoke it with `legalogy devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.legalogy/legalogy.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Legalogy
...
Legalogy **blocks** Control UI connections without device identity.
