...
```bash
legalogy tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `legalogy tui --deliver`
...
- Check the Gateway logs: `legalogy logs --follow`.
- Confirm the agent can run: `legalogy status` and `legalogy models status`.
...
- No agents in picker: check `legalogy agents list` and your routing config.
