...
read_when:
  - You need detailed behavior for legalogy onboard
...
This page is the full reference for `legalogy onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.legalogy/legalogy.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `legalogy doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `legalogy pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `legalogy health`.
    - `legalogy status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.legalogy/.env` so launchd can read it.
...
- OAuth credentials: `~/.legalogy/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.legalogy/agents/<agentId>/agent/auth-profiles.json`

<Note>
Headless and server tip: complete OAuth on a machine with a browser, then copy
`~/.legalogy/credentials/oauth.json` (or `$LEGALOGY_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`legalogy onboard`](/cli/onboard)
