...
read_when:
  - Designing workspace memory (~/.legalogy/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Legalogy integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.legalogy/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.legalogy/workspace` as canonical human-readable memory.
...
```
~/.legalogy/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Legalogy**, but keep a separable core library.

### Why integrate into Legalogy?

- Legalogy already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `legalogy memory recall "…" --k 25 --since 30d`
  - `legalogy memory reflect --since 7d`
...
Pragmatic take for `~/.legalogy/workspace`:
