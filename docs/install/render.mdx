...
Deploy Legalogy on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/legalogy/legalogy)
...
repository configures everything needed to run Legalogy:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: LEGALOGY_STATE_DIR
        value: /data/.legalogy
      - key: LEGALOGY_WORKSPACE_DIR
        value: /data/workspace
      - key: LEGALOGY_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/legalogy`.
...
If you use the original Legalogy repository, Render will not auto-deploy your Legalogy. To update it, run a manual Blueprint sync from the dashboard.
...
For Legalogy, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Legalogy host.
