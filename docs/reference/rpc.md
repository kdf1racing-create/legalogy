...
Legalogy integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Legalogy owns lifecycle when `channels.signal.autoStart=true`.
...
- Legalogy spawns `imsg rpc` as a child process (legacy iMessage integration).
