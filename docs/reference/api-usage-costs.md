...
Legalogy features that can generate provider usage or paid API calls.
...
- `legalogy status --usage` and `legalogy channels list` show provider **usage windows**
...
Legalogy can pick up credentials from:
...
- `legalogy status --usage`
- `legalogy models status --json`
...
`legalogy models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
