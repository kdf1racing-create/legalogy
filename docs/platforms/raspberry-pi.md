---
summary: "Legalogy on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Legalogy on a Raspberry Pi
  - Running Legalogy on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Legalogy on Raspberry Pi

## Goal

Run a persistent, always-on Legalogy Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Legalogy features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled legalogy

# Enable if not
sudo systemctl enable legalogy

# Start on boot
sudo systemctl start legalogy
```
...
```bash
# Check logs
journalctl -u legalogy --no-pager -n 100

# Common fix: rebuild
cd ~/legalogy  # if using hackable install
npm run build
sudo systemctl restart legalogy
```
