---
summary: "Use MiniMax M2.1 in Legalogy"
read_when:
  - You want MiniMax models in Legalogy
...
```bash
legalogy plugins enable minimax-portal-auth  # skip if already loaded.
legalogy gateway restart  # restart if gateway is already running
legalogy onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/legalogy/legalogy/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `legalogy.json`:
...
- Use `legalogy models list` and `legalogy models set minimax/MiniMax-M2.1` to switch.
...
- Running `legalogy configure` and selecting **MiniMax M2.1**, or
