---
summary: "Use Venice AI privacy-focused models in Legalogy"
read_when:
  - You want privacy-focused inference in Legalogy
...
```bash
legalogy onboard --auth-choice venice-api-key
```
...
```bash
legalogy chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Legalogy shows all available Venice models. Pick based on your needs:
...
```bash
legalogy models set venice/claude-opus-45
legalogy models set venice/llama-3.3-70b
```

List all available models:

```bash
legalogy models list | grep venice
```

## Configure via `legalogy configure`

1. Run `legalogy configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Legalogy automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
legalogy chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
legalogy chat --model venice/claude-opus-45

# Use uncensored model
legalogy chat --model venice/venice-uncensored

# Use vision model with image
legalogy chat --model venice/qwen3-vl-235b-a22b

# Use coding model
legalogy chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `legalogy models list` to see currently available models. Some models may be temporarily offline.
