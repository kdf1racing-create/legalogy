...
read_when:
  - Setting up Twitch chat integration for Legalogy
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Legalogy connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
legalogy plugins install @legalogy/twitch
```

Local checkout (when running from a git repo):

```bash
legalogy plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `LEGALOGY_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "legalogy", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use LEGALOGY_TWITCH_ACCESS_TOKEN env var)
...
```bash
LEGALOGY_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
