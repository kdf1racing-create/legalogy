...
Matrix is an open, decentralized messaging protocol. Legalogy connects as a Matrix **user**
...
```bash
legalogy plugins install @legalogy/matrix
```

Local checkout (when running from a git repo):

```bash
legalogy plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `legalogy plugins install @legalogy/matrix`
   - From a local checkout: `legalogy plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Legalogy calls the same
     login endpoint, stores the access token in `~/.legalogy/credentials/matrix/credentials.json`,
...
- On first connection, Legalogy requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Legalogy logs a warning.
...
`~/.legalogy/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `legalogy pairing list matrix`
  - `legalogy pairing approve matrix <CODE>`
...
- On startup, Legalogy resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
