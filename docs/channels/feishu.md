...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Legalogy to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
legalogy plugins install @legalogy/feishu
```

Local checkout (when running from a git repo):

```bash
legalogy plugins install ./extensions/feishu
```
...
If you just installed Legalogy, run the wizard:
...
2. Configuring app credentials in Legalogy
...
1. You already ran `legalogy channels add` for Feishu
2. The gateway is running (`legalogy gateway status`)
...
```bash
legalogy pairing approve feishu <CODE>
```
...
2. Run `legalogy logs --follow` and look for `chat_id`
...
2. Run `legalogy logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `legalogy gateway status`  | Show gateway status           |
| `legalogy gateway install` | Install/start gateway service |
| `legalogy gateway stop`    | Stop gateway service          |
| `legalogy gateway restart` | Restart gateway service       |
| `legalogy logs --follow`   | Tail gateway logs             |
...
4. Check logs: `legalogy logs --follow`
...
5. Ensure the gateway is running: `legalogy gateway status`
6. Check logs: `legalogy logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.legalogy/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.legalogy/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
