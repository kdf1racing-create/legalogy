...
```bash
legalogy plugins install @legalogy/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
legalogy plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Legalogy" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `legalogy pairing list nextcloud-talk`
  - `legalogy pairing approve nextcloud-talk <CODE>`
