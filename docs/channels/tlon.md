...
Tlon is a decentralized messenger built on Urbit. Legalogy connects to your Urbit ship and can
...
```bash
legalogy plugins install @legalogy/tlon
```

Local checkout (when running from a git repo):

```bash
legalogy plugins install ./extensions/tlon
```
...
By default, Legalogy blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `legalogy message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Legalogy replies in-thread.
