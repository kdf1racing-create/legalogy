...
```bash
legalogy plugins install @legalogy/msteams
```

Local checkout (when running from a git repo):

```bash
legalogy plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
Legalogy will offer the local install path automatically.
...
3. Configure Legalogy with those credentials.
...
- Talk to Legalogy via Teams DMs, group chats, or channels.
...
- On startup, Legalogy resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.legalogy/legalogy.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Legalogy, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `legalogy-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `legalogy plugins install @legalogy/msteams`
   - From a local checkout: `legalogy plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Legalogy in Teams", "full": "Legalogy in Teams" },
...
Legalogy handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Legalogy only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/LegalogyShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Legalogy sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `legalogy message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.legalogy/msteams-polls.json`.
...
```bash
legalogy message send --channel msteams \
...
legalogy message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
legalogy message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
legalogy message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
legalogy message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
