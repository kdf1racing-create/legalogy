...
- Publish as `legalogy/plugin-sdk` (or export from core under `legalogy/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `LegalogyPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: LegalogyConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: LegalogyConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: LegalogyConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: LegalogyConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: LegalogyConfig): string;
  };
};
```
...
- Introduce `legalogy/plugin-sdk`.
- Add `api.runtime` to `LegalogyPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `legalogyRuntime: ">=2026.2.0"`).
