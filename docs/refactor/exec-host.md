...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.legalogy/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.legalogy/node.json` (node id + pairing token).
...
Path: `~/.legalogy/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.legalogy/exec-approvals.sock",
...
- Unix socket at `~/.legalogy/exec-approvals.sock` (0600).
