...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Legalogy
...
Running Legalogy via Node with `tsx` fails at startup with:

```
[legalogy] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `legalogy status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch legalogy.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node legalogy.mjs status` works on Node 25.
