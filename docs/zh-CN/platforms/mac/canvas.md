...
- `~/Library/Application Support/Legalogy/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `legalogy-canvas://<session>/<path>`

示例：

- `legalogy-canvas://main/` → `<canvasRoot>/main/index.html`
- `legalogy-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `legalogy-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
legalogy nodes canvas present --node <id>
legalogy nodes canvas navigate --node <id> --url "/"
legalogy nodes canvas eval --node <id> --js "document.title"
legalogy nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__legalogy__/a2ui/
```
...
legalogy nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
legalogy nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "legalogy://agent?message=Review%20this%20design";
```
