...
- **会话存储**（聊天历史 + 路由状态）位于 `~/.legalogy/agents/<agentId>/sessions` 下。
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.legalogy/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 状态目录：`~/.legalogy`（或 `LEGALOGY_STATE_DIR`）
- 工作区：`~/.legalogy/workspace`（或 `~/.legalogy/workspace-<agentId>`）
- 智能体目录：`~/.legalogy/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
- 会话：`~/.legalogy/agents/<agentId>/sessions`

### 单智能体模式（默认）

如果你什么都不做，Legalogy 运行单个智能体：

- `agentId` 默认为 **`main`**。
- 会话键为 `agent:main:<mainKey>`。
- 工作区默认为 `~/.legalogy/workspace`（或当设置了 `LEGALOGY_PROFILE` 时为 `~/.legalogy/workspace-<profile>`）。
- 状态默认为 `~/.legalogy/agents/main/agent`。
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认：~/.legalogy/credentials/whatsapp/personal
