...
你可以在 `~/.legalogy/legalogy.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/legalogy.log"
  }
}
```
...
```bash
legalogy channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.legalogy/legalogy.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/legalogy/legalogy-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- Legalogy 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
LEGALOGY_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `legalogy plugins enable diagnostics-otel` 启用插件。
...
- `legalogy.tokens`（计数器，属性：`legalogy.token`、`legalogy.channel`、`legalogy.provider`、`legalogy.model`）
- `legalogy.cost.usd`（计数器，属性：`legalogy.channel`、`legalogy.provider`、`legalogy.model`）
- `legalogy.run.duration_ms`（直方图，属性：`legalogy.channel`、`legalogy.provider`、`legalogy.model`）
- `legalogy.context.tokens`（直方图，属性：`legalogy.context`、`legalogy.channel`、`legalogy.provider`、`legalogy.model`）

消息流：

- `legalogy.webhook.received`（计数器，属性：`legalogy.channel`、`legalogy.webhook`）
- `legalogy.webhook.error`（计数器，属性：`legalogy.channel`、`legalogy.webhook`）
- `legalogy.webhook.duration_ms`（直方图，属性：`legalogy.channel`、`legalogy.webhook`）
- `legalogy.message.queued`（计数器，属性：`legalogy.channel`、`legalogy.source`）
- `legalogy.message.processed`（计数器，属性：`legalogy.channel`、`legalogy.outcome`）
- `legalogy.message.duration_ms`（直方图，属性：`legalogy.channel`、`legalogy.outcome`）

队列 + 会话：

- `legalogy.queue.lane.enqueue`（计数器，属性：`legalogy.lane`）
- `legalogy.queue.lane.dequeue`（计数器，属性：`legalogy.lane`）
- `legalogy.queue.depth`（直方图，属性：`legalogy.lane` 或 `legalogy.channel=heartbeat`）
- `legalogy.queue.wait_ms`（直方图，属性：`legalogy.lane`）
- `legalogy.session.state`（计数器，属性：`legalogy.state`、`legalogy.reason`）
- `legalogy.session.stuck`（计数器，属性：`legalogy.state`）
- `legalogy.session.stuck_age_ms`（直方图，属性：`legalogy.state`）
- `legalogy.run.attempt`（计数器，属性：`legalogy.attempt`）

### 导出的 span（名称 + 关键属性）

- `legalogy.model.usage`
  - `legalogy.channel`、`legalogy.provider`、`legalogy.model`
  - `legalogy.sessionKey`、`legalogy.sessionId`
  - `legalogy.tokens.*`（input/output/cache_read/cache_write/total）
- `legalogy.webhook.processed`
  - `legalogy.channel`、`legalogy.webhook`、`legalogy.chatId`
- `legalogy.webhook.error`
  - `legalogy.channel`、`legalogy.webhook`、`legalogy.chatId`、`legalogy.error`
- `legalogy.message.processed`
  - `legalogy.channel`、`legalogy.outcome`、`legalogy.chatId`、`legalogy.messageId`、`legalogy.sessionKey`、`legalogy.sessionId`、`legalogy.reason`
- `legalogy.session.stuck`
  - `legalogy.state`、`legalogy.ageMs`、`legalogy.queueDepth`、`legalogy.sessionKey`、`legalogy.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `legalogy doctor`。
