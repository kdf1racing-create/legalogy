...
- `/usage cost` in chats: local cost summary aggregated from Legalogy session logs.
- CLI: `legalogy status --usage` prints a full per-provider breakdown.
- CLI: `legalogy channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
