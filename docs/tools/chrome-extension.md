---
summary: "Chrome extension: let Legalogy drive your existing Chrome tab"
read_when:
...
The Legalogy Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate legalogy-managed Chrome profile.
...
Legalogy then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Legalogy release (npm package) as static files. There is no separate “build” step.

After upgrading Legalogy:

- Re-run `legalogy browser extension install` to refresh the installed files under your Legalogy state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Legalogy ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Legalogy to control.
...
- `ON`: attached; Legalogy can drive that tab.
...
Debugging: `legalogy sandbox explain`
...
`legalogy browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `legalogy browser extension install` first to copy the extension to a stable location under your Legalogy state directory.
...
- **This is not isolated** like the dedicated legalogy-managed profile.
