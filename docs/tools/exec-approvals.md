...
- **gateway host** → `legalogy` process on the gateway machine
...
`~/.legalogy/exec-approvals.json` directly.

CLI: `legalogy approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
