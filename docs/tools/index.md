---
summary: "Agent tool surface for Legalogy (browser, canvas, nodes, message, cron) replacing legacy `legalogy-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `legalogy-*` skills
title: "Tools"
---

# Tools (Legalogy)

Legalogy exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `legalogy-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `legalogy.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Legalogy logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:legalogy`: all built-in Legalogy tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`legalogy node run`).
...
- Requires a Brave API key (recommended: `legalogy configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Legalogy-managed browser.
...
- Quick smoke: `legalogy nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `legalogy gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, Legalogy clamps `tools.sessions.visibility` to `tree`.
