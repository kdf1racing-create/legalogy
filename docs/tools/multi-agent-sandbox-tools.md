...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `legalogy sandbox explain`.
...
Legacy `agent.*` configs are migrated by `legalogy doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=legalogy-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${LEGALOGY_STATE_DIR:-$HOME/.legalogy}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
