...
4. Users browse, download, and install skills in Legalogy.
...
If you want to add new capabilities to your Legalogy agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
4. Start a new Legalogy session so it picks up the new skill.
...
## How it fits into Legalogy

By default, the CLI installs skills into `./skills` under your current working directory. If a Legalogy workspace is configured, `clawhub` falls back to that workspace unless you override `--workdir` (or `CLAWHUB_WORKDIR`). Legalogy loads workspace skills from `<workspace>/skills` and will pick them up in the **next** session. If you already use `~/.legalogy/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches Legalogy how to perform a
...
Interested in becoming a moderator? Ask in the Legalogy Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to Legalogy workspace).
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/legalogy/skills` and `~/.legalogy/skills`). This is designed to find older skill installs without extra flags.
