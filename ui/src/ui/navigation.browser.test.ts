...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/legalogy/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/legalogy");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/legalogy/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__LEGALOGY_CONTROL_UI_BASE_PATH__ = "/legalogy";
    const app = mountApp("/legalogy/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/legalogy");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/legalogy/sessions");
  });
