...
export async function handleWhatsAppStart(host: LegalogyApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: LegalogyApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: LegalogyApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: LegalogyApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: LegalogyApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: LegalogyApp): string {
...
function resolveGatewayHttpAuthHeader(host: LegalogyApp): string | null {
...
function buildGatewayHttpHeaders(host: LegalogyApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: LegalogyApp) {
...
export function handleNostrProfileToggleAdvanced(host: LegalogyApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: LegalogyApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: LegalogyApp) {
  const state = host.nostrProfileFormState;
