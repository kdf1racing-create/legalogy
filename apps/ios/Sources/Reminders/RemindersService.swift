...
final class RemindersService: RemindersServicing {
    func list(params: LegalogyRemindersListParams) async throws -> LegalogyRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[LegalogyReminderPayload], Error>) in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return LegalogyReminderPayload(
        ...
        return LegalogyRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: LegalogyRemindersAddParams) async throws -> LegalogyRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = LegalogyReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return LegalogyRemindersAddPayload(reminder: payload)
    }
