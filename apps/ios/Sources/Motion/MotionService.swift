...
final class MotionService: MotionServicing {
    func activities(params: LegalogyMotionActivityParams) async throws -> LegalogyMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[LegalogyMotionActivityEntry], Error>) in
        ...
        return LegalogyMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: LegalogyPedometerParams) async throws -> LegalogyPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<LegalogyPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = LegalogyPedometerPayload(
