...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__LEGALOGY_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<LegalogyTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = LegalogyTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> LegalogyTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return LegalogyTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> LegalogyTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> LegalogyTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return LegalogyTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> LegalogyTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = LegalogyTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: LegalogyTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: LegalogyAgentEventPayload.self) else {
                continue
            }
