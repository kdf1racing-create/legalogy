...
final class ContactsService: ContactsServicing {
    ...
    func search(params: LegalogyContactsSearchParams) async throws -> LegalogyContactsSearchPayload {
        ...
        return LegalogyContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: LegalogyContactsAddParams) async throws -> LegalogyContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return LegalogyContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return LegalogyContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> LegalogyContactPayload {
        LegalogyContactPayload(
