...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> LegalogyNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> LegalogyNetworkStatusPayload {
        let status: LegalogyNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> LegalogyNetworkStatusPayload {
        LegalogyNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
