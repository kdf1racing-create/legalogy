...
This is an **alpha** iOS app that connects to an Legalogy Gateway as a `role: node`.
...
3. On iOS: Legalogy → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project Legalogy.xcodeproj -scheme Legalogy -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/LegalogyKit` contains the shared transport/types used by the iOS app.
