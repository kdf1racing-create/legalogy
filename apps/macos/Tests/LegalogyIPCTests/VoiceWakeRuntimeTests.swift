...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["legalogy"]
        let text = "hey legalogy"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey legalogy do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["legalogy"], minPostTriggerGap: 0.3)
