...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: LegalogyProtocol.AnyCodable(["ok": LegalogyProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: LegalogyProtocol.AnyCodable(["ok": LegalogyProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = LegalogyProtocol.AnyCodable([
            "runId": LegalogyProtocol.AnyCodable("run-1"),
            "sessionKey": LegalogyProtocol.AnyCodable("main"),
            "state": LegalogyProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: LegalogyProtocol.AnyCodable(["a": LegalogyProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
