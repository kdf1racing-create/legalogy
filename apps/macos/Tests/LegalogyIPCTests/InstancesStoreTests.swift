...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: LegalogyProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: LegalogyProtocol.AnyCodable] = [
            "presence": .init([LegalogyProtocol.AnyCodable(entry)]),
        ]
        let payload = LegalogyProtocol.AnyCodable(payloadMap)
