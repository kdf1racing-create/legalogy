...
enum CommandResolver {
    private static let projectRootDefaultsKey = "legalogy.gatewayProjectRootPath"
    private static let helperName = "legalogy"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let legalogyEntry = root.appendingPathComponent("legalogy.mjs").path
        if FileManager().isReadableFile(atPath: legalogyEntry) { return legalogyEntry }
        let binEntry = root.appendingPathComponent("bin/legalogy.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/legalogy")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let legalogyPaths = self.legalogyManagedPaths(home: home)
        if !legalogyPaths.isEmpty {
            extras.insert(contentsOf: legalogyPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + legalogyPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func legalogyManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".legalogy"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func legalogyExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectLegalogyExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("legalogy.mjs").path,
            root.appendingPathComponent("bin/legalogy.js").path,
        ]
    ...
    static func hasAnyLegalogyInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.legalogyExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func legalogyNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let legalogyPath = self.projectLegalogyExecutable(projectRoot: root) {
                return [legalogyPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "legalogy", subcommand] + extraArgs
            }
            if let legalogyPath = self.legalogyExecutable(searchPaths: searchPaths) {
                return [legalogyPath, subcommand] + extraArgs
            }

            let missingEntry = """
            legalogy entrypoint missing (looked for dist/index.js or legalogy.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real legalogy CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/legalogy"
        ...
        \(cliSection)
        \(projectSection)
        if command -v legalogy >/dev/null 2>&1; then
          CLI="$(command -v legalogy)"
          legalogy \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/legalogy.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/legalogy.mjs"
            node "$PRJ/legalogy.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/legalogy.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/legalogy.js"
            node "$PRJ/bin/legalogy.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent legalogy"
          pnpm --silent legalogy \(quotedArgs);
        else
          echo "legalogy CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? LegalogyConfigFile.loadDict()
