...
private let deepLinkLogger = Logger(subsystem: "ai.legalogy", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "Legalogy is paused", message: "Unpause Legalogy to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run Legalogy agent?", message: body) else { return }
        }
