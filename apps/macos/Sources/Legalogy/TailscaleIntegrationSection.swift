...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.legalogy/legalogy.json. Prefer LEGALOGY_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.legalogy/legalogy.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.legalogy/legalogy.json. Restart the gateway to apply."
        }
