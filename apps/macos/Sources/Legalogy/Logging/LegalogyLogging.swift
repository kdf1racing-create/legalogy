...
enum LegalogyLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = LegalogyOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = LegalogyFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        LegalogyLogging.bootstrapIfNeeded()
        let label = LegalogyLogging.makeLabel(subsystem: subsystem, category: category)
...
struct LegalogyOSLogHandler: LogHandler {
...
struct LegalogyFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = LegalogyLogging.parseLabel(self.label)
