...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.legalogy.mac`) even if the shipped
    ...
    {
        if environment["LEGALOGY_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "legalogy.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "legalogy.nixMode") { return true }
