...
final class CanvasWindowController: NSWindowController, WKNavigationDelegate, NSWindowDelegate {
    ...
    init(sessionKey: String, root: URL, presentation: CanvasPresentation) throws {
        ...
        (() => {
          try {
            ...
            if (globalThis.__legalogyA2UIBridgeInstalled) return;
            globalThis.__legalogyA2UIBridgeInstalled = true;
            ...
            globalThis.addEventListener('a2uiaction', (evt) => {
              try {
                ...
                const handler = globalThis.webkit?.messageHandlers?.legalogyCanvasA2UIAction;
                ...
                const hasBundledA2UIHost =
                  !!globalThis.legalogyA2UI ||
                  !!document.querySelector('legalogy-a2ui-host');
                ...
                location.href = 'legalogy://agent?' + params.toString();
              } catch {}
            }, true);
          } catch {}
        })();
    ...
    func applyDebugStatusIfNeeded() {
        ...
        (() => {
          try {
            const api = globalThis.__legalogy;
            if (!api) return;
    ...
    func snapshot(to outPath: String?) async throws -> String {
        ...
        if let outPath, !outPath.isEmpty {
            ...
            path = "/tmp/legalogy-canvas-\(CanvasWindowController.sanitizeSessionKey(self.sessionKey))-\(ts).png"
        }
