...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["LEGALOGY_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/legalogy")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("legalogy-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("legalogy-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/legalogy/ (rolling or stdout), or nil if none exist.
