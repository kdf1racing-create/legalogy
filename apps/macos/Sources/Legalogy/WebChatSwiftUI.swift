...
private let webChatSwiftLogger = Logger(subsystem: "ai.legalogy", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: LegalogyChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> LegalogyChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(LegalogyChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> LegalogyChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                LegalogyGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    LegalogyAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<LegalogyChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any LegalogyChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = LegalogyChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: LegalogyChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<LegalogyChatView>) -> NSViewController
    {
