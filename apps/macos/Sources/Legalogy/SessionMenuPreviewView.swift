...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.legalogy", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> LegalogySessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: LegalogySessionPreviewEntry,
    ...
    private static func cache(payload: LegalogySessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: LegalogyChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [LegalogyKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [LegalogyKit.AnyCodable]) -> [LegalogyChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(LegalogyChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: LegalogyChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: LegalogyChatMessage) -> Bool {
    ...
    private static func toolNames(for message: LegalogyChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: LegalogyChatMessage) -> String? {
