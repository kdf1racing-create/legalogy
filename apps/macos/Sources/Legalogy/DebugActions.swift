...
enum DebugActions {
    private static let verboseDefaultsKey = "legalogy.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "legalogy.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = LegalogyPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Legalogy", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = LegalogyPaths.configURL
        guard
