...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.legalogy", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: LegalogyPeekabooBridgeServices?
    private static var legalogySocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Legalogy", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = LegalogyPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.legalogySocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.legalogySocketPath, privacy: .public)")
    }
...
@MainActor
private final class LegalogyPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.legalogy.peekaboo")
