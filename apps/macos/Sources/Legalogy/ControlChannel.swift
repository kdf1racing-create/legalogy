...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: LegalogyProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.legalogy", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: LegalogyKit.AnyCodable]()) {
                $0[$1.key] = LegalogyKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: LegalogyProtocol.AnyCodable?) -> [String: LegalogyProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: LegalogyProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: LegalogyKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: LegalogyProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("legalogy.control.heartbeat")
    static let controlAgentEvent = Notification.Name("legalogy.control.agent")
}
