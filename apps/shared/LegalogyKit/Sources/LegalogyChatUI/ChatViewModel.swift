...
private let chatUILogger = Logger(subsystem: "ai.legalogy", category: "LegalogyChatUI")

@MainActor
@Observable
public final class LegalogyChatViewModel {
    public private(set) var messages: [LegalogyChatMessage] = []
    ...
    public var attachments: [LegalogyPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [LegalogyChatPendingToolCall] = []
    public private(set) var sessions: [LegalogyChatSessionEntry] = []
    private let transport: any LegalogyChatTransport
    ...
    private var pendingToolCallsById: [String: LegalogyChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any LegalogyChatTransport) {
    ...
    public var sessionChoices: [LegalogyChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [LegalogyChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: LegalogyPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [LegalogyChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: LegalogyChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func messageIdentityKey(for message: LegalogyChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [LegalogyChatMessage],
        incoming: [LegalogyChatMessage]) -> [LegalogyChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [LegalogyChatMessage]) -> [LegalogyChatMessage] {
        var result: [LegalogyChatMessage] = []
    ...
    private static func dedupeKey(for message: LegalogyChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [LegalogyChatMessageContent] = [
            LegalogyChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> LegalogyChatAttachmentPayload in
            LegalogyChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> LegalogyChatSessionEntry {
        LegalogyChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: LegalogyChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: LegalogyChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: LegalogyAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = LegalogyChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> LegalogyPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
