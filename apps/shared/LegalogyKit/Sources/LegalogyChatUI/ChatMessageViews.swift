...
struct ChatMessageBubble: View {
    let message: LegalogyChatMessage
    let style: LegalogyChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? LegalogyChatTheme.userText : LegalogyChatTheme.assistantText
    ...
    private var inlineAttachments: [LegalogyChatMessageContent] {
    ...
    private var toolCalls: [LegalogyChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [LegalogyChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? LegalogyChatTheme.userBubble
        }
        if self.style == .onboarding {
            return LegalogyChatTheme.onboardingAssistantBubble
        }
        return LegalogyChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return LegalogyChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: LegalogyChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? LegalogyChatTheme.userText : LegalogyChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: LegalogyChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(LegalogyChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(LegalogyChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [LegalogyChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: LegalogyChatTheme.assistantText)
            }
        }
    }
}
