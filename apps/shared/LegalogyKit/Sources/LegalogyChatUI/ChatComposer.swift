...
@MainActor
struct LegalogyChatComposer: View {
    @Bindable var viewModel: LegalogyChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(LegalogyChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(LegalogyChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \LegalogyPendingAttachment.id)
                { (att: LegalogyPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            LegalogyPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(LegalogyChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(LegalogyChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(LegalogyChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(LegalogyChatTheme.subtleCard)
        .clipShape(Capsule())
    }
