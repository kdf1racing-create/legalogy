...
import Foundation
import Testing
@testable import LegalogyChatUI
...
private final class TestChatTransport: @unchecked Sendable, LegalogyChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [LegalogyChatHistoryPayload]
    private let sessionsResponses: [LegalogyChatSessionsListResponse]

    private let stream: AsyncStream<LegalogyChatTransportEvent>
    private let continuation: AsyncStream<LegalogyChatTransportEvent>.Continuation

    init(
        historyResponses: [LegalogyChatHistoryPayload],
        sessionsResponses: [LegalogyChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<LegalogyChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> LegalogyChatHistoryPayload {
        ...
        return self.historyResponses.last ?? LegalogyChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [LegalogyChatAttachmentPayload]) async throws -> LegalogyChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return LegalogyChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> LegalogyChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? LegalogyChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: LegalogyChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = LegalogyChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = LegalogyChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { LegalogyChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = LegalogyChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = LegalogyChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = LegalogyChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { LegalogyChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
