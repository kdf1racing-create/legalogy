...
This document describes how the Legalogy ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`legalogy acp` exposes an ACP agent over stdio and forwards prompts to a running
Legalogy Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Legalogy Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `legalogy acp` over stdio.

Example config:

```bash
legalogy config set gateway.remote.url wss://gateway-host:18789
legalogy config set gateway.remote.token <token>
```

Example run:

```bash
legalogy acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
legalogy acp --session agent:main:main
legalogy acp --session agent:design:main
legalogy acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Legalogy ACP” to start a thread.

## Execution Model

- ACP client spawns `legalogy acp` and speaks ACP messages over stdio.
...
```bash
legalogy acp --session agent:main:main
legalogy acp --session-label "support inbox"
legalogy acp --reset-session
```
...
`legalogy acp` resolves the Gateway URL and auth from CLI flags or config:
